 ...
 *
 * The arguments must have the same type, with the usual exception for integer
 * numbers. Moreover, the second one must be a number.
 * 
 * \param e The environment of the definition
 * \param t1 The first argument. Must be of type ::MSAT_REAL or ::MSAT_INT
 * \param t2 The second argument. Must be a number of type
 *           ::MSAT_REAL or ::MSAT_INT
 * \return The term (t1 / t2), or a t s.t. ::MSAT_ERROR_TERM(t) is true
 *         in case of errors.
 */
...
/**
 * \brief Dumps the given \a term in msat format to the given output file
 *
 * \param e The environment in which \a term is defined
 * \param term The term to convert
 ...
 * \return zero on success, nonzero on error.
 */ 
int msat_to_msat_file(msat_env e, msat_term term, FILE *out);

/**
 * \brief Dumps the given \a term in SMT-LIB format to the given output file
 *
 * \param e The environment in which \a term is defined
 * \param term The term to convert
 * \param out The output file
 * \return zero on success, nonzero on error.
 */ 
int msat_to_smtlib_file(msat_env e, msat_term term, FILE *out);
 ...
 * elements, which must be free()'d by the user. NULL is returned in case of

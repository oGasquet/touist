...
module Make (L : LABEL) = struct
 ...
 let rec parse vars =
  let ident = (Parser.many1 (Parser.noneof " \t\n\r()\",") >>= function
   | (['-';'>'] | ['=';'='] | ['-';'>';'='] | ['-';'>';'<';'-']) as i ->
     Parser.failwith ("'" ^ String.of_char_list i ^ "' is not a legal identifier")
   | i -> Parser.return i) <?> "identifier"
  in
  let create_fun id ar =
   if List.mem id vars then
     Parser.failwith ("'" ^ id ^ "' was declared as variable, but is used as function symbol")
   else
     let id = String.xml_entities_encode id in
     Parser.get_state >>= fun s ->
     try 
       let (f, s) = S.create_fun ar id s in
       Parser.set_state s >> Parser.return f
     with S.Arity -> Parser.failwith ("'" ^ id ^ "' is used with incorrect arity")
  in
  let create_var id =
   let id = String.xml_entities_encode id in
 ...
 let rename =
  let rec rename m = function
   ...
   | Fun (f,ts) ->
    let g ti (ts,m) = M.liftM (P.apply (flip List.cons ts) id) (rename m ti) in
 ...
 (* Positions *)
 (* Beware that Rule.overlaps relies on the fact that if the root is included
  * in the result list, it will be the first element. *)
 ...
 let rec count_var v = function
   ...
   | Fun (f, ts) -> List.foldl (fun a t -> a + count_var v t) 0 ts
 ;;

...
module type RULE = sig
 ...
 val parallel_overlaps : ?inner:bool -> ?outer:bool -> t list -> t -> ((t * Position.t) list * t) list m
 val compare : t -> t -> int
 val equal : t -> t -> bool
 val of_string : string list -> string -> t m
 val of_xml_string : string -> t m
...
module type TRS = sig
 ...
 val rewrites : term -> Position.t -> t -> term -> bool
 val critical_peak : (rule * Position.t * rule) -> (term * term * term) m
 val critical_pairs : ?inner:bool -> ?outer:bool -> t -> t -> (term * term) list m
 val parallel_critical_peak : ((rule * Position.t) list * rule) -> (term * term * term) m
 val parallel_critical_pairs : ?inner:bool -> ?outer:bool -> t -> t -> (term * term) list m
 val overlaps : ?inner:bool -> ?outer:bool -> t -> t -> (rule * Position.t * rule) list m
 val var_overlaps : t -> (rule * Position.t * rule) list m
 val parallel_overlaps : ?inner:bool -> ?outer:bool -> t -> t -> ((rule * Position.t) list * rule) list m
 val copy : t -> t
 val depth : t -> int
 val hash : t -> int
 val size : t -> int

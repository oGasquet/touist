...
module type STATE = sig
 ...
 that this function does ***not*** change the old signature. *)
 ...
 @raise Arity if [s] contains already a function symbol with name [n] but
 different arity. *)
...
module type MONAD = sig
 ...
 (** [create_fun a n] is equivalent to {!val: Rewriting.STATE.create_fun} except
 that the result is encapsulated in a monad. If there exists already a function
 symbol with name [n] but different arity, the returned monad represents the
 exception [Arity]. Note that as a side effect the old signature is changed too.
 *)
...
module type POSITION = sig
 ...
 val parallel_subsets : t list -> t list list
 (** [parallel_subsets ps] returns the non-empty sublists of [ps] that
 consist of mutually parallel positions. *)
...
module type TERM = sig
 ...
 val count_var : Variable.t -> t -> int
 (** [count_var v t] returns the number of occurrences of [v] in [t]. *)
...
module type SUBSTITUTION = sig
 ...
 (** [is_renaming s] checks whether [s] is a variable renaming. *)
...
*)
 ...
 val renaming : term list -> substitution m
 (** [renaming u] computes a renaming [s] with respect to the terms [u]. Note
...
module type RULE = sig
 ...
 val is_right_nonerasing : t -> bool
 ...
 Not tail-recursive. As a side effect the underlying signature changes. *)
 val is_overlap : t -> Position.t -> t -> bool m
 (** [is_overlap r p r'] checks if [r] and [r'] are overlapping at
 position [p] 
 Not tail-recursive. Note that as a side effect the
 underlying signature changes. *)
 ...
 position [p]. It works for extra-variable TRSs, too. *)
 ...
 val overlaps : ?inner:bool -> ?outer:bool -> t -> t -> (t * Position.t * t) list m
 (** [overlaps ~inner:inner ~outer:outer r r'] computes positions of [r']
 that cause an overlap with [r]. If [outer] is set, root overlaps are
 included in the result; if [inner] is set, non-root overlaps are included
 in the result. If neither flag is set, the result will be empty. Both flags
 are set by default. Rewrite rules [r] and [r'] overlap if there is a
 ...
 val var_overlaps : t -> t -> (t * Position.t * t) list m
 (** [var_overlaps r r'] computes overlaps at variable positions. 
 see overlaps for details. *)
 val parallel_overlaps : ?inner:bool -> ?outer:bool -> t list -> t
  -> ((t * Position.t) list * t) list m
 (** [parallel_overlaps ~inner:inner ~outer:outer rs r'] computes parallel
 overlaps of rules from [rs] with [r']. If [outer] is set, root overlaps are
 included in the result; if [inner] is set, non-root overlaps are included
 in the result. If neither flag is set, the result will be empty. Both flags
 are set by default. A parallel overlap [(os,r1)] with [r1] consists of a
 non-empty set [os] of pairs of positions of [r1] and rules from [rs], such
 that all positions  are pairwise parallel, and the unification problem that
 equates the subterm at a position [p] of [r1] unifies with the left-hand
 side of a variant of the rule [r'] for each [(r',p)] in [os], is solvable.
 Changes the underlying signature. *)
...
module type TRS = sig
 ...
 val find_option : (rule -> bool) -> t -> rule option
 ...
 val critical_peak : (rule * Position.t * rule) -> 
  (term * term * term) m
 (** [critical_peak overlap] returns the peak [(t,s,u)] with [t <- s -> u]
 corresponding to [overlap]. Changes the signature. *)
 val critical_pairs : ?inner:bool -> ?outer:bool -> t -> t
  -> (term * term) list m
 (** [critical_pairs ~inner:inner ~outer:outer r s] computes critical pairs
 between [r] and [s]. If [outer] is set, outer (root) critical pairs are
 included in the result; if [inner] is set, inner (non-root) critical pairs
 are included in the result. If neither flag is set, the result will be
 empty. Both flags are set by default. This function is not tail-recursive,
 and changes the signature. *)
 val parallel_critical_peak : ((rule * Position.t) list * rule) ->
  (term * term * term) m
 (** [parallel_critical_peak overlap] returns the peak [(t,s,u)] with
 [t <- s -> u] corresponding to [overlap]. Changes the signature. *)
 val parallel_critical_pairs : ?inner:bool -> ?outer:bool -> t -> t
  -> (term * term) list m
 (** [parallel_critical_pairs ~inner:inner ~outer:outer r s] computes parallel
 critical pairs between [r] and [s]. If [outer] is set, outer parallel critical
 pairs are included in the result; if [inner] is set, inner parallel critical
 pairs are included in the result. If neither flag is set, the result will be
 empty. Both flags are set by default. This function changes the signature. *)
 val overlaps : ?inner:bool -> ?outer:bool -> t -> t
  -> (rule * Position.t * rule) list m
 (** [overlaps ~inner:inner ~outer:outer r s] computes overlaps between
 rewrite rules in [r] and [s]. If [outer] is set, root overlaps are
 included in the result; if [inner] is set, non-root overlaps are included
 in the result. If neither flag is set, the result will be empty. Both flags
 are set by default. Rewrite rules [u] and [v] overlap if there is a position
 ...
 val var_overlaps : t -> (rule * Position.t * rule) list m
 (** [var_overlaps r] computes overlaps at variable positions.
 see [overlaps] for details. *)
 val parallel_overlaps : ?inner:bool -> ?outer:bool -> t -> t
  -> ((rule * Position.t) list * rule) list m
 (** [parallel_overlaps ~inner:inner ~outer:outer r s] computes parallel
 overlaps of rules from [r] with rules from [s]. If [outer] is set, root
 overlaps are included in the result; if [inner] is set, non-root overlaps
 are included in the result. If neither flag is set, the result will be
 empty. Both flags are set by default. Changes the underlying signature. *)
...
module type REWRITE = sig
 ...
 val step_make : trs -> term -> position -> rule -> step
 (** [step_make t p r] creates a rewrite step *)
 val step_clear_lab : step -> step
 (** [step_clea_lab s] clears the label of a rewrite step *)

 val step_get_term : step -> term
 (** [step_get_term s] return starting term from rewrite step [s] *)
 val step_get_redex : step -> term
 (** [step_get_redex s] returns the redex from rewrite step [s] *)
 val step_get_reduct : step -> term
 (** [step_get_reduct s] returns reduct from rewrite step [s] *)
 ...
 val step_get_lab : step -> label list
 (** [step_get_lab s] returns labels associated to step [s] *)
 val step_add_label : ?left:bool -> step -> label -> step
 (** [step_add_label s lab] adds label [lab] to step [s] *)
 ...
 *)
 ...
 val sequences : ?s:t -> ?n:int -> ?width:int -> term -> trs -> term list list
 (** [sequences ~s:s ~n:n ~width:w t] returns all rewrite sequences of length
 up to [n] starting from [t]; the returned lists have length up to [n+1] and
 start with the final reduct. If [w>=0] then the rewrite tree is restricted
 to width [w]. *)
 val are_joinable : ?s:t -> ?n:int -> ?width:int -> term -> term -> trs -> bool
 (** [are_joinable ~s:s ~n:n ~width:width u v r] checks if the terms [u] and
 [v] are joinable with respect to the TRS [r] within [n] steps using the
 rewrite strategy [s]. If [n] is smaller than [0] this function may not
 terminate. Per default [s = Full] and [n = ~-1]. The meaning of [width]
 is the same as in [reducts] *)
 val is_reachable : ?s:t -> ?n:int -> term -> term -> trs -> bool
 (** [is_reachable ~s:s ~n:n u v r] checks if from term [u] we reach term 
 [v] respect to the TRS [r] within [n] steps using the rewrite
 strategy [s]. If [n] is smaller than [0] this function may not
 terminate. Per default [s = Full] and [n = ~-1]. *)
 val is_reversible: rule -> trs -> bool
 (** [is_reversible r t] checks if a rule r in a TRS t is reversible (rhs of r
 arrives the lhs of r with rules in t) within 10 rewrite steps. *)
 val find_normalform : ?n:int -> term -> trs -> term
 (** [find_normalform ~n:n term trs] returns the first normal
  form in [term] found within [n] reductions. If no normal form is found,
  a [n] step reduct of [term] is returned instead. *)
 val reducts : ?s:t -> ?n:int -> ?width:int -> term -> trs -> term list
 (** [reducts ~n:n ~width:width u r] computes all reducts of the term [u] with
 respect to the TRS [r] that can be reached within [n] steps, limiting the
 number of i-step reducts to [width] if [width >= 0]. If [n] is smaller than
 [0] this function may not terminate. Per default [s = Full] and [n = ~-1].
 ...
 *)
 val parallel : trs -> term -> term list
 (** [parallel trs t] returns reducts of [t] with one parallel step *)
 ...
 val min_joins : ?s:t -> ?m:int -> term -> term -> trs -> (term list * term list) list option
 (** [min_joins ~s:s ~m:m t u trs] computes the joining sequences of minimal 
 length (plus [m]). Returns [None] if [t] and [u] are not joinable. May
 not terminate if [trs] is nonterminating.
 Default for [s] is [Full], default for [m] is [0].*)
end
...
module type DIAGRAM = sig
 ...
 (*** VALUES ******************************************************************)
 val critical_diagrams : ?m:int -> trs -> t list option m
 (** [critical_diagrams trs] computes the (minimal+m) critical diagrams *)
 val par_critical_diagrams : ?m:int -> trs -> t list option m 
 (** [par_critical_diagrams trs] computes the (minimal+m) parallel critical
     diagrams (note, only proper parallel critical peaks are considered) *)
 val critical_pair : t -> term * term
 (** [critical_pair d] returns the (parallel) critical pair represented by [d] *)
 val peak : t -> step * step
 (** [peak d] returns the (local) peak of [d] *)
 val par_peak : t -> step list * step
 (** [par_peak] returns the (local) peak of [d], where the left step consists
     of a list of (mutually parallel) steps from the source *)
 val joins : t -> (step list * step list) list
 (** [joins d] returns the (minimal) peak of [d] *)
 val label : (t -> int -> step -> (int * step)) -> int -> t -> t
 (** [label f n d] labels diagram [d] according to labeling function [f]
 and initial label [n] *)
 val make : (step * step) -> (step list * step list) list -> t
 (** [make p js] returns a diagram with peak [p] and joins [js] *)
 val map : (step -> step) -> t -> t
 (** [map f t] applies [f] to every step in [d] *)
 val mirror : t -> t
 (** [mirror d] mirrors [d] *)
 val source : t -> term
 (** [source d] returns the source of [d] *)
 val take : ?n:int -> t -> t
 (** [take n d] takes the peak and the first [n] steps of teh joins of [d] *)
  
 (** Predicates **)
 val is_decreasing : t -> bool
 (** [is_decreasing d] tests if [d] is decreasing *)
 val is_decreasing_strict : t -> bool
 (** [is_decreasing_strict d] tests if [d] is strict decreasing *)
 val is_par_decreasing : (t -> 'a -> step -> ('a * step)) -> 'a -> t -> bool
 (** [is_par_decreasing f a d] tests if [d] is parallel decreasing,
     given a labeling function. *)
 val is_parallel_closed : trs -> t -> bool
 (** [is_parallel_closed trs d] tests if [d] is parallel closed (Huet-Toyama) *)
 val is_strongly_closed : ?n:int -> trs -> t -> bool
 (** [is_strongly_closed n trs d] tests if [d] is strongly closed (Huet) with at most [n] steps *)

 val filter : (t -> bool) -> t -> t
 (** [filter p d] drops those joins for which [d] does not satisfy [p] *)
 val find : (t -> bool) -> t -> t
 (** [find p d] finds one join that satisfies [p]. 
  By default a shortest join is selected. 
  @raise Failure "hd" if no join satisfies [p]. *)

 (** Printers **)
 val fprintfm : Format.formatter -> t -> unit m
 (** [fprintfm fmt t] pretty prints [t] into [fmt] *)
 val fprintfm_peak : Format.formatter -> t -> unit m
 (** [fprintfm fmt t] pretty prints the peak of [t] into [fmt] *)
...
module type MONADX = sig
 ...
 val freeze : ('a -> 'b t) -> ('a -> 'b) t
 (** [freeze f] evaluates to a pure function that executes the action [f]
 with the state at the time of the call to [freeze]. That is, the function
 is frozen in time. *)

...
module Make (L : LABEL) = struct
 ...
 let find = List.find;;
 let find_option = List.find_option;;
 ...
 let reverse = map Rule.reverse <?> "not an SRS";;
 ...
 let unique r = List.unique r;;

 let add_nv rule r = add rule (filter (not <.> Rule.is_variant rule) r);;
 let variant_free r = of_list (List.foldr add_nv empty (to_list r));;
 ...
 let is_srs r = is_trs r & for_all Rule.is_string r;;
 ...
 (* Confluence *)
 let overlaps ?(inner = true) ?(outer = true) r s =
  M.flat_map (flip M.flat_map s <.> Rule.overlaps ~inner:inner ~outer:outer) r;;

 let var_overlaps r = M.flat_map (flip M.flat_map r <.> Rule.var_overlaps) r;;

 let parallel_overlaps ?(inner = true) ?(outer = true) r s =
  M.flat_map (fun s' -> Rule.parallel_overlaps ~inner:inner ~outer:outer r s') s
 ;;

 let critical_peak (r1,p,r2) = 
 ...
 let critical_pairs ?(inner = true) ?(outer = true) r s =
  let cp o =
   critical_peak o >>= fun (t,_,u) -> 
   (* let cpl = Rule.rewrite s p r1 in *)
   (* let cpr = Rule.rewrite s Position.root r2 in *)
   (* let l = Rule.lhs r and l' = Rule.lhs r' and r' = Rule.rhs r' in *)
   (* let cpl = Sub.apply_term s (Term.replace p (Rule.rhs r) l') in *)
   (* let cpr = Sub.apply_term s r' in *)
   M.return (t,u)
  in
  overlaps ~inner:inner ~outer:outer r s >>= fun ols ->
  M.map cp ols >>= fun cps ->
  M.return (List.unique cps) 
 ;;

 let parallel_critical_peak (os,r1) =
  M.map (fun (r,p) -> Rule.rename r >>= fun r' ->
                      M.return (r', p)) os >>= fun os' ->
  let mu = E.unify_problem (List.map (fun (r',p) ->
       (Rule.lhs r',Term.subterm p (Rule.lhs r1))) os') in
  let s = Sub.apply_term mu (Rule.lhs r1) in
  let t = List.foldl (fun s' (r',p) ->
       Term.replace p (Sub.apply_term mu (Rule.rhs r')) s') s os' in
  let u = Sub.apply_term mu (Rule.rhs r1) in
  M.return (t,s,u)
 ;;

 let parallel_critical_pairs ?(inner = true) ?(outer = true) r s =
  let pcp o = parallel_critical_peak o >>= fun (t,_,u) -> M.return (t,u) in
  parallel_overlaps ~inner:inner ~outer:outer r s >>= fun ols ->
  M.map pcp ols >>= fun pcps ->
  M.return (List.unique pcps)
 ;;

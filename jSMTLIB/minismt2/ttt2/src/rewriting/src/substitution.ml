...
module Make (L : LABEL) = struct
 ...
 let is_renaming s =
   let swap_vars x t subst =
     match t with
     | Var y -> replace y (Var x) subst
     | _ -> raise Not_renaming
   in
   try
     let rev_s = fold swap_vars s empty in
     size s = size rev_s
   with Not_renaming -> false
 ;;

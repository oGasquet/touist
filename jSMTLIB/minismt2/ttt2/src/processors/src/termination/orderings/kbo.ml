...
let spec =
 let spec = [
  ...
  ("-w0",Arg.Set_int flags._w0,
   "Set w0 to fixed value (w0 = 0 means value not fixed).");
 ] in
 Arg.alignx 80 spec
;;
...
let fprintfx_redpair p = XO.node "orderingConstraintProof" [XO.node "redPair" [
  XO.node "knuthBendixOrder" [
    XO.int "w0" (Number.to_int (WF.w0 p.wf));
    WF.fprintfx p.wf (Prec.to_list p.precedence);
    fprintfx_af p.af
  ]
]];;
...
let fprintfx fs p =
  ...
  if dp || Problem.is_rp p.input || Problem.is_sp p.input then (
    ...
    failwith "Kbo.fprintfx: XML output not supported"
  )
;;
...
let weight_fs rule = 
 ...
 liftM C.big_sum xs >>= fun sl ->
 liftM C.big_sum (sequence (List.map weight wr)) >>= fun sr ->
 return (sl, sr)
;;
...
let c_af f = arity f >>= fun a ->
 af_l f $&$ gen_and (fun i -> liftM Logic.neg (af_p f i)) a
;;
...
let ge_ur op ge rule = liftM2 op (u(Rule.lhs rule)) (kbo_ge rule);;
...
let encode s w = get >>= fun c ->
 if !(flags._w0) <= 0 then (fresh_arith >>= fun w0 -> c.w0 := w0; return ();)
 else (c.w0 := Logic.of_int !(flags._w0); return ());
 let fs = (Trs.funs (Trs.union s w)) in
...
let solve signature fs p = 
 ...
 if not (P.is_sp p || P.is_rp p || P.is_dp p || P.is_cp p ||
        ((P.is_ddp p || P.is_auxp p) && not (Trs.is_empty (P.get_strict p)))) then
 None else
 ...
 Logic.run ~obits:!(flags.ob) (
  ...
  Made.lift (Logic.solve ~solver:c.solver ~goal:goal phi) >>= function
   | None -> return None
   | Some ass ->

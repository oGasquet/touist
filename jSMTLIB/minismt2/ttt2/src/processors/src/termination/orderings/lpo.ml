...
let fprintfx fs p =
  ...
  if dp || Problem.is_rp p.input || Problem.is_sp p.input then (
    ...
    failwith "Lpo.fprintfx: XML output not supported"
  )
;;
...
let ge_ur op ge rule = 
 liftM2 op (u(Rule.lhs rule)) (ge rule)
;;
...
let solve signature fs p = 
 ...
 Logic.run (
  ...
  Made.lift (Logic.solve ~solver:c.solver phi) >>= function
   | None -> return None
   | Some ass ->
...
let (>>=) = M.(>>=);;
let solve fs p =
 if P.is_sp p || P.is_rp p || P.is_cp p || 
 ((P.is_ddp p || P.is_auxp p) && not (Trs.is_empty (P.get_strict p))) then
  M.get >>= fun s -> M.return (solve s fs p)
 else M.return None
;;

...
let coeff_of_int n = M.make 1 1 (Logic.of_int n);;
...
let dummy make f = 
 ...
 match !t_itp with
  ...
  | "exp" -> (match fn with
   ...
   | "add"  -> makel [a2;a1;a1] v2
   | "mul"  -> makee [a0;a0;a0] v2 (makel [a1;a0;a0] v2) [a0;a1;a0] v2
   | _ -> make_fresh 2 (List.concat v2) f
)
  | "append" -> (match fn with
   | "false"    -> makel [a2] v0
   | "append"   -> makee [a0;a0;a0] v2 (makee [a0;a0;a2] v2 (makel [a1;a1;a1] v2) [a0;a0;a1] v2) [a0;a0;a1] v2
   | "ifappend" ->
      (* make_fresh 2 (List.concat v3) f *)
      (* makel [a1;a1;a1;a3] v3  *)
     makee [a1;a1;a1;a3] v3 (makee [a0;a0;a0;a2] v3 (makel [a2;a1;a0;a1] v3) [a0;a0;a0;a2] v3) [a0;a0;a0;a0] v3
   | "is_empty" -> makel [a1;a0] v1 
   | "cons" -> makel [a1;a1;a0] v2 
   | "tl" -> makel [a2;a0] v1 
   | "hd" -> makel [a1;a0] v1 
   | _ -> make_fresh 4 (List.concat v2) f
)
...
let pre geq f g = greater_zero_poly (q g) <->> geq (exp f) (exp g);;
let case1 cmp f g = greater_zero_poly (q f) <&> cmp (scale (base ()) (exp f)) (Right g);;
let case2 f g = geq_poly (p f) (p g) <&> geq_poly (q f) (q g);;
let case3 cmp f g =
   let pp = diff_poly (lin_exp f) (lin_exp g) in
   let mm = (mul_poly (low pp) (q f)) in
   let l2 = P.add (p f) (mul_poly (low (lin_exp g)) mm) in
   let r2 = P.add (p g) (mul_poly (low (lin_exp g)) (q g)) in
   geq_poly mm (q g) <&>
   geq_poly pp poly_zero <&> geq_poly (q' (exp f)) (q' (exp g)) <&> cmp l2 r2
;;
...
let rec geq f g = match f,g with
  ...
  | Right f, Right g ->
    pre geq f g <&> (case1 geq f g <|> case2 f g <|> case3 geq_poly f g)
;;
...
let rec gt f g = match f,g with
  ...
  | Right f, Right g ->
   let pre = pre geq f g in
   let c2 = (greater_zero_poly (q f) <&> gt (exp f) (exp g)) <|> gt_poly (p f) (p g) <|> (gt_poly (q f) (q g)) in
   (pre <&> (case1 gt f g <|> (case2 f g <&> c2) <|> case3 gt_poly f g))
;;

...
*)
 ...
 let gen_ij f a = List.map f (List.square (List.range 0 a));;

 let fprintf_intp ppf (fs,a,ms,const) =
  let xs = List.gen (fun i -> Format.sprintf "x%d" i) a in
  let xxs = gen_ij (fun (i,j) -> Format.sprintf "x%dx%d" i j) a in
  ...
  let s = match fs with 
   | "" -> ""
   | _ -> Format.sprintf "[%s]%s%s%s = " fs lp xlist rp
  in

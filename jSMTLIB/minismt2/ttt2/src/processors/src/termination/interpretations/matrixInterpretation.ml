...
let spec =
 ...
 let spec = [
  ...
  bound");
  ("-lredex",Arg.Set flags.lredex,"confluence: label steps for redex system");
...
let rec eval_const d i = function
 ...
 | Term.Fun (f,ts) ->
  let (cs,c) = 
   ...
   (* namely, we have encountered a function symbol that was only used
    * in rules that have already been oriented earlier. We could pick an
    * arbitrary interpretation for the current component. *)
...
let fprintf_orient d i fmt p =
 let fprintf_rule d i fmt rule =
  interpret_term_ann d i (Rule.lhs rule) >>>>= fun l ->
  interpret_term_ann d i (Rule.rhs rule) >>>>= fun r ->
...
let fprintf_flags fmt p = 
 ...
 if (p.plstar || p.plredex) then 
  Format.fprintf fmt ", %s lab=%s " 
  (if p.plstar then "star" else if p.plredex then "redex" else "")
  (if p.pleft then "left" else "right");
;;
...
let decode_intp ass p = 
 let (s,w) = Problem.get_sw p in
 decode_intp ass (Trs.union s w)
;;
...
let compare_int i1 i2 =
 let minus_i2 = NP.make (NP.map (fun m -> let (m,vs) = NMono.split m in
  NMono.make (NM.scale (Number.of_int ~-1) m) vs) i2) in
 let diff = NP.add i1 minus_i2 in
 let ge = List.for_all (fun i -> Number.ge i Number.zero)
  (List.flat_map NM.to_list (NP.coefficients diff)) in
 let gt = Number.ge (NM.get 0 0 (NMono.coefficient (NP.constant_part diff)))
  Number.one in
 if ge then if gt then Order.GT else Order.EQ else Order.NONE
;;

let labelm lab left i (d, _, s) = 
 let (ls, r) = Diagram.par_peak d in
 Monad.map (lab i) ls >>>>= fun lls ->
 lab i r >>>>= fun lr -> lab i s >>>>= fun ls ->
 let lls = List.map (fun l -> compare_int l ls) lls in
 let ll = if List.exists Order.is_gt lls then Order.GT
     else if List.exists Order.is_eq lls then Order.EQ else Order.NONE in
 let lr = compare_int lr ls in
 let lab = Number.to_int (NM.get 0 0 (NMono.coefficient (NP.constant_part ls))) in
 Monad.return (-1, Rewrite.step_add_label ~left:left s (ll, lr, lab))
;;

let label_label f ass ip p =
 decode_intp ass ip >>= fun i ->
 lift_monad (Monad.freeze (f i)) >>= fun f ->
 let d = !(flags.dim) in
 return (Problem.label_cds (fun dd i n -> flags.dim := d; f (dd,i,n)) (~-1) p)
;;

let lab_starm dim i s =
 Term.spine (Rewrite.step_get_term s) (Rewrite.step_get_pos s) >>>>= fun u ->
 Monad.return (eval_const dim i u) 
;;

let lab_redexm dim i s =
 Monad.return (Rewrite.step_get_redex s) >>>>= interpret_term dim i
;;

let decode_lstar left dim ass ip p =
 label_label (labelm (lab_starm dim) left) ass ip p
;;

let decode_lredex left dim ass ip p =
 label_label (labelm (lab_redexm dim) left) ass ip p
;;
 
let decode ass ip = get >>= fun c -> 
 let (strict,weak) = Problem.get_sw ip in
 (if !(flags.inc) = None then (
  ...
  return (Problem.set_sw s (Trs.union w0 w1) ip)
 ) else match Option.the !(flags.inc) with
   | A (* fallthrough *)
   | B -> return (Problem.set_dps Trs.empty ip)
   | C -> decode_compress_edge ass ip
   | E -> decode_direct_edge ass ip
   | N -> decode_direct_node ass ip
 ) >>= fun p ->
 if !(flags.cp) then (
  let s',w' = Problem.get_sw p in
 ...
 if !(flags.lstar) then decode_lstar !(flags.left) !(flags.dim) ass ip p else return p >>= fun p ->
 if !(flags.lredex) then decode_lredex !(flags.left) !(flags.dim) ass ip p else return p >>= fun p ->
 return p
;;

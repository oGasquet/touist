...
module Approximation = struct
 ...
 let modify f g =
  let of_term = uncurry R.of_terms and combine l = M.liftM (pair l) in
  let apply f g r = f (R.lhs r) >>= fun l -> combine l (g (R.rhs r)) in
  let add r m = apply f g r >>= fun r -> M.liftM (Trs.add (of_term r)) m in
  M.liftM Trs.unique <.> Trs.fold add (M.return Trs.empty)
 ;;
 
 let strong =
  modify (M.lift <.> Term.ren) (const (M.liftM Term.make_var M.fresh_var))
 ;;

 let rename = modify (M.lift <.> Term.ren) (M.lift <.> Term.ren);;
 ...
 let linearize l =
  ...
  M.lift (Term.ren ~p:(const <.> not <.> flip L.mem ps) l)
 ;;
 
 let adapt l r =
  let hd ps = try L.hd ps with Failure _ -> Pos.root in
  let check x = 1 >= Pos.length (hd (Term.var_pos x l)) in
  let add s x = M.liftM (flip (Sub.add x) s <.> Term.make_var) M.fresh_var in
  let update s x = if check x then M.return s else add s x in
  M.liftM (flip Sub.apply_term r) (M.foldl update Sub.empty (Term.vars r))
 ;;
 
 let growing =
  let add r m =
   let l = R.lhs r and r = R.rhs r in
   linearize l >>= fun l -> M.liftM (R.of_terms l) (adapt l r) >>= fun r ->
   M.liftM (Trs.add r) m
  in
  M.liftM Trs.unique <.> Trs.fold add (M.return Trs.empty)
 ;;
...
let comment =
 "Remove all edges from the current DG that are not contained in the \
  ...
  regularity preserving approximations of the given TRS)."
;;
...
let approximate dps trs t =
 ...
 let fs = L.union (L.diff (Trs.funs dps) (Trs.roots dps)) (Trs.funs trs) in
 (match !(flags.a) with
  | Growing -> M.liftM (pair Reach.predecessors) (growing trs)
  | Rename -> M.liftM (pair Reach.successors) (rename trs)
  | Strong -> M.liftM (pair Reach.successors) (strong trs)) >>= fun (f,trs) ->
...
let compute p =
 ...
 let (ls,rs) = L.map R.lhs (G.in_nodes g),L.map R.rhs (G.out_nodes g) in
 ...
 let add trs m r = M.liftM (flip (Map.add r) m) (approximate dps trs r) in
 M.foldl (add trs) Map.empty ls >>= fun ml ->
 M.foldl (add itrs) Map.empty rs >>= fun mr ->
 (* compute arcs *)
 filter_edges ml mr (G.restrict (Trs.to_list dps) g) >>= fun h ->
 M.get >>= fun s ->
 ...
 M.return (if progress then Some (p,proof,q) else None)
;;

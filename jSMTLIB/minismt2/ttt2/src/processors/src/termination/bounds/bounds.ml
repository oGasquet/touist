...
(* flags_qd should be part of the 'flags' record, but then we cannot refer
 * to it from the 'Completion' module *)
let flags_qd = ref false;
...
module Completion = struct
 ...
 let paths cp_s cp_w cp_sharp e_s e_w e_sharp ht rc qd qc s w sharp x =
  let max n f = M.liftM (max n) (M.lift (Monad.get_height f)) in
  M.foldl max 0 (A.funs (get_automaton x)) >>= fun n ->
  let update get_h set_h get_vs set_vs e qd trs x =
   let rec update = function
    | [] ->
     (* configurate automaton *)
     (if qd then A.complete ~qc:qc ~rc:rc (get_automaton x) (* FIXME: >>= A.reduce *)
     ...
     (* compute violations *)
     let compatible h = A.compatible ~c:true ~qc:qc ~h:h ~t:(e n) trs a in
 ...
 let compute cp_s cp_w cp_sharp e_s e_w e_sharp ht rc qc s w sharp x =
  ...
  let qd = not (linear && Trs.is_left_linear sharp) || !flags_qd in
 ...
 let paths_sharp m dp rt qc sharp =
  ...
  in
  M.liftM paths_sharp (arity sharp)
 ;;
 ...
 let generate e dp rt ht rc qc s w sharp xs =
  ...
  let generate s w = M.liftM (List.map (compute s w)) (paths s w) in
 ...
 let generate e dp rt ht rc qc s s' w w' sharp xs =
  ...
  let generate s w = M.liftM (List.map (calculate s w)) (paths s w) in
  let compute (f,g,h) = compute f g h e_z e_z e_z ht rc qc s' w' sharp in
  M.liftM (List.map compute) (paths s' w') >>= fun fs' ->
...
type proof = {
 automaton : A.t;
 precedence : (State.t * State.t) list option;
 complexity : bool;
 ...
 let spec = [
  ...
  ("-dp",Arg.Set flags.dp,
   "Uses the enrichments 'match-DP' and 'top-DP' instead of 'match' and \
    'top' if a DP problem is given. Make sure that as enrichment either \
  ...
  ("-e",Arg.String set_enrichment,
   "Specifies the enrichment used to show termination. Possible values \
    ...
    instead of 'roof' because soundness of 'roof' is unknown. Per default \
    'match' is used.");
  ...
  ("-qd",Arg.Set flags_qd,
   "Force the use of quasi-deterministic automata. (Only useful for testing.)");
  ("-qc",Arg.Set flags.qc,
  ...
  ("-rfc",Arg.Set flags.rfc,"Uses right-hand sides of forward closures.");
  ("-rt",Arg.Set flags.rt,
  ...
  ("-cert",Arg.Set flags.cert,
   "Produce certifiable (state-compatible) output.")]
 in
 Arg.alignx 80 spec
;;
...
let plain e dp rt all ht s w =
 ...
 if all && !(flags.l) = Constructor then
  ...
  if !(flags.dp) then
   let init r = M.liftM (init (Trs.singleton r)) (instances r s w) in
...
let rfc e dp rt all ht s w =
 M.lift (Trs.linear s) >>= fun s' ->
 M.lift (Trs.linear w) >>= fun w' ->
 M.lift (Trs.sharp (Trs.union s' w')) >>= fun (f,sharp) ->
 let init s a = Completion.init s a !(flags.steps) in
 ...
 let create s = M.liftM (init s) (I.specific f s) in
 (if all then M.liftM List.singleton (create s)
...
let make_cert s w a0 =
 let ll = Trs.is_left_linear s && Trs.is_left_linear w in
 let qd = A.is_quasi_det a0 in
 if not (ll || qd) then failwith "[not certifiable]";
 if qd then
  let (a, ec) = A.qdet a0 in
  let a = A.map_trans Rhs.det a in
...
let strategy e dp rt all ht s w =
 ...
 (if !(flags.rfc) && rl then rfc else plain) e dp rt all ht s w >>= function
  ...
  | Some x ->
   M.get >>= fun (_,status) ->
   let aut = Completion.get_automaton x in
   let (aut, eps) = if !(flags.cert) then make_cert s w aut else (aut, None) in
   let proof = make_proof aut eps e status in
...
(* FIXME: provide this function in standard interface *)
let fresh_fun n =
  Monad.fresh_fun   >>= fun f ->
  Monad.add_ari f n >>= fun _ ->
  Monad.return f
;;
...
let solve fs p =
 ...
 if P.for_all Trs.is_trs p then
  ...
  else if (P.is_ddp p || P.is_auxp p) && not (Trs.is_empty (P.get_strict p))
   then Monad.adopt (eval rp)
...
let fprintf fs fmt p =
 let fprintf fs fmt p =
  ...
  Option.fold (fun ec ->
    F.fprintf fmt "@]@\n@\n@[<1>precedence:@\n";
    List.iter (fun (l,r) ->
      F.fprintf fmt "%a -> %a@\n" State.fprintf l State.fprintf r
    ) ec;
  ) () proof.precedence;
  F.fprintf fmt "@]@\n@[<1>problem:@\n";
  M.lift (P.fprintfm fmt (get_op p)) >>= fun _ ->
  F.fprintf fmt "@]@\n"; M.lift (List.hd fs fmt) >>= fun _ ->
...
let fprintfx_enrichment e = XO.node "type" [match e with
  | Roof  -> XO.leaf "roof"
  | Match -> XO.leaf "match"
];;

let fprintfx_final_states ta =
  XO.node "finalStates" (List.map State.fprintfx (A.finals ta))
;;

let fprintfx_precedence ec =
  XO.node "relation" (
    List.map (fun (l, r) ->
      XO.node "entry" [State.fprintfx l; State.fprintfx r]
    ) ec
  )
;;

let fprintfx _ p fmt =
  let prob   = get_ip p in
  let proof  = get_proof p in
  let status = proof.status in
  if P.is_sp prob then (
    (*compute XML printer using the automata monad, but
    do not actually output anything from inside of it*)
    let obtain_fprintfx p =
      A.bound proof.automaton >>= fun b ->
      M.return (XO.node "bounds" ([
        fprintfx_enrichment proof.enrichment;
        XO.int "bound" b;
        fprintfx_final_states proof.automaton;
        A.fprintfx proof.automaton
      ] @ Option.fold (fun ec -> [fprintfx_precedence ec]) [] proof.precedence))
    in
    let (>>=) = Monad.(>>=) in
    Monad.get       >>= fun s ->
    let (fpfx, (state, _)) =
      (*FIXME: (CS) I've no clue about the semantics of status*)
      Either.right (M.eval (s, status) (obtain_fprintfx p))
    in
    Monad.set state >>= fun _ ->
    fpfx fmt
  ) else (
    failwith "Bounds.fprintfx: XML output not supported"
  )
;;

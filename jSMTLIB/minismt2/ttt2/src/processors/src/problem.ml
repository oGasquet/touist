...
type diagram = {orig: Trs.t; cstrict: Trs.t; cweak: Trs.t; cds: Diagram.t list;
 labeling: ((Diagram.t -> int -> Rewrite.step -> (int * Rewrite.step)) * int) list };;
...
(* add_and_label_cds uses the recorded labeling function and applies it to
   the new diagrams. *)
let add_and_label_cds cds p = match p with
 | {p = Diagram p'} ->
...
let rec fprintfm ?(g = false) fmt = function
 ...
 | {p = AuxRT p} as q ->
  ...
  F.fprintf fmt "@]@\n@[<1>weak:@\n"; Trs.fprintfm fmt p.aweak >>= fun _ ->
  F.fprintf fmt "@]@\n@[<1>original problem:@\n";

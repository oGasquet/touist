 ...
 let rec ur_term = function
  ...
  | Fun (f,ts) as t ->
   if H.mem covered t then M.return Trs.empty
    ...
    M.foldl (fun ur t -> M.liftM (Trs.union ur) (ur_term t)) ur ts
 in
 ur_term t
;;
...
let ur dps trs =
 ...
 let union ur r = M.liftM (Trs.union ur) (ur_term covered r trs) in
...
let fprintfx fs p = XO.node "usableRulesProc" [P.fprintfx (get_op p); List.hd fs];;

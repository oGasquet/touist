...
let spec =
 let spec = [
  ...
  ("-nofun",Arg.Set flags.nofun,"Do not introduce a fresh symbol");
 ] in
 Arg.alignx 80 spec
;;
...
let solve fs p = 
  ...
  if P.is_sp p then (
    if !(flags.nofun)
    then failwith(F.sprintf "%s: -nofun only works for DP problems" name)
    else
    ...
    if Trs.is_left_linear w then (
      ...
      if not !(flags.nofun) then (
        ...
        M.return(Some(p,(if w = w'
          ...
          let p = P.set_sw (Trs.of_list s'') (Trs.of_list w'') p in
          (* IMPORTANT! *)
          (* let p = P.update_dg P.Complete p in *)
          p) 
        )
      ) else (
...
let fprintfx fs p =
  (*FIXME: turn flat context closure and root-labeling in two independent
  techniques*)
  let (proof_kind, tag, tag') = if P.is_sp (get_op p)
    then ("trsTerminationProof", "semlab", "flatContextClosure")
    else ("dpProof", "semlabProc", "flatContextClosureProc")
  in
  let semlab = XO.node tag [
    XO.node "model" [XO.leaf "rootLabeling"];
    P.fprintfx (get_op p);
    List.hd fs]
  in
  match get_fcc p with
    | Some (symb, fcs, fcc) -> XO.node tag' [
      (match symb with Some f -> XO.node "freshSymbol" [M.fprintfx_fun f]
                     | None   -> XO.id);
      XO.node "flatContexts" (List.map Ctxt.fprintfx fcs);
      P.fprintfx fcc;
      XO.node proof_kind [semlab]
    ]
    | None -> semlab
;;

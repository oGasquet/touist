...
let modify_int f ints = update Label.is_int (const (Label.make_int_list ints)) f;;
...
let add_int ?(arity = ~-1) f i s = 
 let l = try get_int_list f s with | Not_found -> [] in
 let (f,s) = label ~arity:arity f (Label.Int (i::l)) s in
 (f,s)
;;
...
let drop_int f s = 
  ...
  (*FIXME: the following expression should have type unit*)
...
let rec fprintfx_fun' s fmt f signature =
  ...
  let (n, s) = create_fun_name f s in
  let ls     = labs f s in
  let f      = fprintfx_fun' s in
  let g      = fprintfx_var s in
  F.fprintf fmt "@{<symbol>@{<name>%s@}" n;

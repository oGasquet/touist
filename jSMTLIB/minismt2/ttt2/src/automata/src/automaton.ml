...
module Make (R : REWRITING)
 ...
 module C = struct
  ...
  let add f c = M.lift (add f c);;
  let remove f c = M.lift (remove f c);;
  let find f c = M.lift (find f c);;
 end
 ...
 type t = {
  ...
  dependency : D.t;
  categorization : C.t;
  epsilons : (Lhs.t, Rhs.t) T.t;
  transitions : (Fun.t, (Lhs.t, Rhs.t) T.t)I.t;
 ...
 let find ?(c = false) l a = match l with
  | State _ -> M.return (try Some (T.find_ l a.epsilons) with Not_found -> None)
  | Fun (f,_) ->
   let rec find = function
    | [] -> None
    | f::fs ->
     try Some (T.find_ l (I.find_ f a.transitions)) with Not_found -> find fs
   in
   M.liftM find (if c then C.find f a.categorization else M.return [f])
 ;;
 ...
 let mem ?(c = false) l =
  M.liftM (Option.fold (const true) false) <.> find ~c:c l
 ;;
 ...
 let for_allc_trans c p a =
  ...
  M.return (List.for_all (fun g -> T.for_all p (I.find_ g a.transitions)) fs)
 ;;

 let for_allf_trans f p a =
  try T.for_all p (I.find_ f a.transitions) with Not_found -> true
 ;;

 let exists_trans p a = I.exists (const (T.exists p)) a.transitions;;

 let existsc_trans c p =
  M.liftM not <.> for_allc_trans c (fun l -> not <.> p l)
 ;;
 ...
 let modify modify l r a = match l with
  ...
  | Fun (f,ps) ->
   ...
   let t = try I.find_ f a.transitions with Not_found -> T.create 1000 in
   let t = I.replace_ f (modify l r t) a.transitions in
   M.return {a with categorization = c; transitions = t; quasi_det = false}
 ;;

 let add = modify T.add_;;
 let replace = modify T.replace_;;
 ...
 let remove l a = match l with
  | State _ ->
   M.return {a with epsilons = T.remove_ l a.epsilons; quasi_det = false}
  | Fun (f,ps) ->
   try
    let t = T.remove_ l (I.find_ f a.transitions) in
    (if T.is_empty t then
     M.liftM (flip pair (I.remove_ f a.transitions)) (C.remove f a.categorization)
    else
     M.return (a.categorization,I.replace_ f t a.transitions)) >>= fun (c,t) ->
    M.return {a with categorization = c; transitions = t; quasi_det = false}
   with Not_found -> M.return a
 ;;

 let is_quasi_det t = t.quasi_det;;
 ...
 let foldf_trans g f a d =
  try T.fold f (I.find_ g a.transitions) d with Not_found -> d
 ;;
 ...
 let mapf_trans g f a =
  try
   let t = T.map f (I.find_ g a.transitions) in
   {a with transitions = I.replace_ g t a.transitions; quasi_det = false}
  with Not_found -> a
 ;;
 ...
 let category f a = M.lift (C.category f a.categorization);;
 ...
 let ec_states ps a =
  let rec ec ps qs a =
   ...
   let ps =
    List.foldl (fun ps p ->
     let ec = Rhs.states <.> T.find_ (State p) in
 ...
 let ec ?(r = false) a =
  ...
  if r then {a with epsilons = T.clear_ a.epsilons} else a
 ;;

 let qdet a =
  let aga = agents a in
  (* the precedence is partially encoded in epsilon transitions *)
  let e_inv : (State.t, State.t list) T.t = fold_eps (fun l rs e_inv ->
    Rhs.fold (fun r e_inv ->
     let v = Lhs.to_state l :: try T.find_ r e_inv with Not_found -> [] in
     T.replace_ r v e_inv
    ) rs e_inv
   ) a (T.create 1000)
  (* and partially in the dependencies *)
  in let e_inv : (State.t, State.t list) T.t = D.fold (fun l r e_inv ->
    (* BF: I'm unsure about the next line, but without it, ttt2 produces
     * a wrong proof in one case. (TRS Standard/Rubio_04/koen.xml) *)
    if not (List.is_subset r aga) then e_inv
    else List.foldl (fun e_inv r ->
      let v = try T.find_ r e_inv with Not_found -> [] in
      if List.mem l v || l = r then e_inv else T.replace_ r (l :: v) e_inv
    ) e_inv r
  ) e_inv a.dependency
  (* find transitive closure *)
  in let e_pre p =
   let rec e_pre qs = function
    | [] -> qs
    | p :: ps ->
     let rs = try T.find e_inv p with Not_found -> [] in
     let qs = p :: qs in
     let rs = List.filter (fun p -> not (List.mem p qs || List.mem p ps)) rs in
     e_pre qs (rs @ ps)
   in List.intersect (e_pre [] [p]) aga
  in let e_inv = T.mapi (fun p _ -> e_pre p) e_inv
  in let e_pre p = try T.find e_inv p with Not_found -> if List.mem p aga then [p] else []
  in let e_inv' = List.flat_map (fun p -> List.map (fun q -> (p,q)) (e_pre p)) (states a)
  (* complete lhss w.r.t. the precedence *)
  in let t = fold_trans (fun l r t ->
   let expand : Lhs.t -> Lhs.t list = function
    | Lhs.Fun (f, qs) -> List.map (Lhs.make_fun f)
     (List.times (List.map e_pre qs))
    | _ -> failwith "invalid lhs"
   in List.foldl (fun t l -> match l with
    | Lhs.Fun (f,_) ->
     let v = try I.find_ f t with Not_found -> T.create 1000 in
     let r = try T.find_ l (I.find_ f (a.transitions)) with Not_found -> 
             try let r' = T.find_ l v in 
                 let a0 = match Rhs.agent r with Some a -> a in
                 let a1 = match Rhs.agent r' with Some a -> a in
                 if List.mem a0 (e_pre a1) then r else
                   if List.mem a1 (e_pre a0) then r' else
                     failwith "???"
       with Not_found -> r in
     I.replace_ f (T.replace_ l r v) t
    | _ -> failwith "invalid lhs"
   ) t (expand l)
  ) a (I.create 1000)
  in let final = List.unique_hash (List.flat_map e_pre (finals a)) in
  let a = { a with finals = final; epsilons = T.create 1; transitions = t } in
  (a, e_inv')
 ;;
 ...
 let compute ec qd xs h l r m =
  m >>= fun ps -> try
   ...
   if ys = [] then M.return ps else
    ...
    M.liftM (Option.fold (states filter) []) (find l h) >>= fun us ->
 ...
 let analyse c h qd dir m t a =
  ...
  let generate h = M.liftM (flip Pair.make h) (history_position c t a) in
...
let ps = List.unique_hash ps in
 ...
 let compatible ?(c = false) ?(qc = false) ?(h = None) ?(t = const M.return) trs a =
  let reachable m r p = reachable qc None false false m r p a in
  ...
  Trs.fold (fun r m ->
   ...
   M.foldl (fun x (ls,p) -> M.foldl (fun (vs,m') (l,s) ->
    M.liftM (S.apply_term s) (t l (Term.of_term r)) >>= fun r ->
    let add c = if c then vs else (r,p)::vs in
    M.liftM (Pair.apply add id) (reachable m' r p)) x ls) (vs,m') ls >>=
   (M.return <.> Pair.apply id (Pair.make m))) d trs >>=
   (M.return <.> List.unique_hash <.> fst)
 ;;

 let paths ?(c = false) ?(h = None) ?(f = false) t =
  M.liftM fst <.> paths c h f Path.empty t
 ;;

 (* Miscellaneous *)
 let copy a =
  let size h = 2 * I.length h in
  let copy h = I.fold (fun f -> I.add_ f <.> T.copy) h (I.create (size h)) in
  { finals = a.finals;
 ...
 let reduce a =
  ...
  let rec reduce ps a =
   M.liftM (Pair.apply id List.unique_hash) (fold (fun l r m ->
    if List.is_subset (args l) ps then M.liftM (Pair.apply id (add r)) m
    else m >>= fun (a',ps') -> M.liftM (flip pair ps') (remove l a'))
    a (M.return (copy a,[]))) >>= fun (a',ps') ->
 ...
 let inter a b =
  ...
  let combine_lhs ass l l' = match (l,l') with
   | Fun (f,ps), Fun (_,qs) ->
    M.liftM (Pair.apply id (Lhs.make_fun f <.> List.rev)) (combine ass ps qs)
  ...
  let product_rhs ass r r' =
   ...
   let map = M.liftM (Pair.apply id Option.some) <.> map ass in
  ...
  List.foldl (fun m f ->
   foldf_trans f (fun l r m -> foldf_trans f (fun l' r' m ->
    ...
    M.liftM (pair ass) (add l r c)) b m) a m)
   (M.return (ass,c)) fs >>= fun (ass,c) ->
  M.liftM (flip set_finals c <.> snd) (product ass (finals a) (finals b)) >>=
  (M.liftM (pair ass) <.> M.return (* reduce *))
 ;;
 ...
 let inter a = M.liftM snd <.> inter a;;
 ...
 let union a b =
  ...
  M.liftM (set_finals ps) (combine a b)
 ;;

 (* Predicates *)
 let is_accepted ?(c = false) t = M.liftM ((<>) []) <.> paths ~c:c ~f:true t;;
 let is_empty t = reduce (copy t) >>= fun a -> M.return (finals a = []);;

 let is_reachable ?(c = false) ?(h = None) t p =
  M.liftM fst <.> reachable c h false false Path.empty t p
 ;;

 let is_det t = T.is_empty (t.epsilons) &&
  for_all_trans (fun l r -> Rhs.is_det r) t
 ;;
 ...
 let rewrite ?(c = false) t a =
  ...
  let rec rewrite qd t = match t with
   ...
   | Term.Fun (f,ts) ->
    ...
    if List.for_all Either.is_right xs then
     ...
     let add r = M.liftM (List.rev_append (get_states r)) in
  ...
  in
  M.liftM to_terms (rewrite false t)
 ;;
 ...
 let context ?(c = false) ?(n = 1) t p a =
  ...
  let rec context t p = match t with
   ...
   | Term.Fun (f,ts) ->
    ...
    let evaluate = M.liftM List.unique_hash <.> M.flat_map (uncurry context) in
    M.liftM (choose <.> List.cons [(t,p)]) (M.map evaluate ms)
  in
  context t p
 ;;
 ...
 let solve a =
  let solve l r reps a =
   ...
   M.liftM (pair reps) (replace l r a)
  in
 ...
 let complete rep reps a =
  ...
  M.liftM (pair rep) (M.foldl update a ts)
 ;;
 ...
 let saturate a =
  ...
  let ps = List.unique_hash (List.rev_append ps qs) in
  M.foldl (fun i f -> M.liftM (max i) (M.find_ari f)) 0 fs >>= fun i ->
  let xs = State.combinations i ps and r = Rhs.singleton p in
  M.foldl (fun a f -> 
    M.liftM (fun i -> List.assoc i xs) (M.find_ari f) >>= fun ys ->
 ...
 let det ?(c = false) ?(q = false) a =
  ...
  M.liftM (set_quasi_det <.> map_trans Rhs.det) (quasi_det ~c:c (ec ~r:true a))
 ;;

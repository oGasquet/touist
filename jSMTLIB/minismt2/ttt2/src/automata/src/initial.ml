...
module Make (R : REWRITING)
 ...
 let generate m a =
  let max f _ m = m >>= fun i -> M.liftM (max i) (M.find_ari f) in
 ...
 let specific f g ts =
  let update a t = M.liftM Term.choose (A.rewrite t a) >>= function
   | Var _ -> M.fail "variable occured"
   | State p -> M.return (A.add_final p a)
   | Fun _ as t ->
    let modify a p = M.liftM (A.add_final p) (g t p a) in
 ...
 let instances f g fs gs ts =
  ...
  let update q a = function
   | State p -> M.return (A.add_final p a)
   | t ->
    let modify a p = M.liftM (A.add_final p) (g (transform q t) p a) in
    f t >>= M.foldl modify a
  in
  if ts = [] then M.return (A.create 1000) else
   ...
   let add (q,a) p = M.liftM (Pair.make q) (A.update (Lhs.State p) q a) in
 ...
 let normal fs trs =
  ...
  M.lift (R.fresh_var) >>= fun x -> M.fresh_state >>= fun p ->
  M.foldl (fun (xs,ps) t ->

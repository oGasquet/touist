...
type solver =
 ...
 | PicoSat  (** use [PicoSat] as back-end *)
 | Gpw of string list
...
type core = a list * bool *
  (?fprintf_a:(Format.formatter -> a -> unit) -> Format.formatter -> unit)
(** type for an unsatisfiable core which consists of:
  - arithmetic formulas in core ({!type: t})
  - flag whether unsatisfiability is due to overflow constraints
  - debugging output function
*)

type t
(** internal type representing result of computation *)
...
val to_string_a : a -> string
(** [to_string_a a] returns pretty output of [a] as string. *)
val to_string_p : p -> string
(** [to_string_p p] returns pretty output of [p] as string. *)
val fprintf_smt : ?logic:string -> ?source:string -> Format.formatter -> p -> unit
(** [fprintf_smt ppf p] writes [p] into [ppf] in SMT-LIB v1.2 format. *)
val fprintf_smt2 : ?logic:string -> ?source:string -> Format.formatter -> p -> unit
(** [fprintf_smt2 ppf p] writes [p] into [ppf] in SMT-LIB v2.0 format. *)
...
(** [cache_arith tbl spec k] associates an arithmetical variable with
    ...
    [spec]. *)
val get_spec : a -> arith
(** [get_spec a] returns the arithmetic specification of [a]. *)
val update_spec : (arith -> arith) -> a -> a
(** [update_spec f a] modifies the arithmetic specification of [a]
    according to [f]. *)
val variables : a -> a list
(** [variables a] returns a list of variables in [a]. *)

(** {2 Solving and Solution Extraction} *)

val solve : ?solver:solver -> ?goal:a option -> ?cnf:bool -> p -> assignment option Monad.t
(** [solve ~solver:s ~goal:g ~cnf:cnf f] 
 - tries to solve formula [f] (using solver [s] (default is [MiniSat]))
 - goal function (which is minimized) is only supported by [MiniSatP]
 - avoids cnf transformation if (if [cnf] is true)
 - returns [None] if [f] is not satisfiable and [Some assign] otherwise *)
val solve2 : ?solver:solver -> ?goal:a option -> ?cnf:bool -> p -> t Monad.t
(** [solve2 ~solver:s ~goal:g f]
 - tries to solve formula [f] (using solver [s] (default is [MiniSat]))
 - goal function (which is minimized) is only supported by [MiniSatP]
 - returns the result of the computation.
   Use {!Logic.get_assignment} to extract an assignment. *)
...
(** [get_assignment r] returns [Some ass] if the computation [r] contains
    the assignment [ass] and [None] otherwise. *)
val get_unsat_core : t -> core option
...
val set_print_stat: unit -> unit
(** [set_print_stat ()] for internal use *)

...
let add_side_constraint f = get >>= fun s ->
 Hashtbl.add s.State.sc f ();
 set s
;;

let add_overflow_constraint f =
 get >>= fun s ->
 Hashtbl.add s.State.overflow_constraints f ();
 set s
;;

let get_side_constraint () = get >>= fun s ->
 let acc = F.top in
 let acc = Hashtbl.fold (fun k _ acc -> k <&> acc) s.State.sc acc in
 let acc = Hashtbl.fold (fun k _ acc -> k <&> acc) s.State.overflow_constraints acc in
 return acc
;;

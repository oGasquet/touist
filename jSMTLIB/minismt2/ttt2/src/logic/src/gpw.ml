...
let send tuples rhs ineq =
 ...
 GpwSolver.add_constraint v c rhs ineq 
;;
...
let rec tf = function
 | F.Top          -> return [top]
 | F.Bot          -> return [bot]
 | F.And (x,y)    -> liftM2 (@) (tf x) (tf y)
 | F.Eq (a,F.C b) -> sequence [liftM2 eq (t a) (return (to_int b))]
 | F.Ge (a,F.C b) -> sequence [liftM2 ge (t a) (return (to_int b))]
 | F.Gt (a,F.C b) -> sequence [liftM2 gt (t a) (return (to_int b))]
(*
...
and t = function
 ...
 | F.Add (a, b) -> liftM2 (@) (t a) (t b)
 | F.Sub (a, b) -> liftM2 (@) (t a) (liftM invert (t b))
 | F.Mul (F.C i, b) -> liftM2 scale (return (to_int i)) (t b)
 (*
...
let rec val_of ass = function
 | F.Top          -> return true
 | F.Bot          -> return false
 | F.And (x,y)    -> liftM2 (&&) (val_of ass x) (val_of ass y)
 | F.Eq (a,F.C b) -> liftM2 Number.eq (val_of2 ass a) (return b)
 | F.Ge (a,F.C b) -> liftM2 Number.ge (val_of2 ass a) (return b)
 | F.Gt (a,F.C b) -> liftM2 Number.gt (val_of2 ass a) (return b)
(*
 | F.P _ -> failwith "prop variable not allowed"
 | F.Not _ -> failwith "negation not allowed"
 | F.Or (x, y) -> failwith "disjunction not allowed"
...
and val_of2 ass = function
 ...
 | F.Add (a, b)     -> liftM2 Number.add (val_of2 ass a) (val_of2 ass b)
 | F.Sub (a, b)     -> liftM2 Number.sub (val_of2 ass a) (val_of2 ass b)
 | F.Mul (F.C a, b) -> liftM2 Number.mul (return a) (val_of2 ass b) 
 (*
 | F.Fresh a -> failwith "how to represent fresh?"
...
let solve ?(goal=None) ?(options=[]) phi = 
 tf phi >>= fun pbcs ->
 Option.fold (fun gf -> liftM2 cons (liftM mn (t gf)) (return pbcs)) (return pbcs) goal 
  >>= fun pbcs ->
 let mode = Option.fold (const 0) 1 goal in
 let n_var = List.length (variables pbcs) in
 let n_pbc = List.length pbcs in
 GpwSolver.init n_var n_pbc mode; 
 List.iter GpwSolver.setOption options;
 List.iter send_constraint pbcs;
 try
  liftM Option.some (build_ass (GpwSolver.solve ()))
 with 
  | Failure _ -> 

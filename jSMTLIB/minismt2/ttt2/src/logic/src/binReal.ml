...
(*** conversions **************************************************************)
let fprintfh highlight ppf (c, d) =
 match d = BinRat.zero with
 | true -> BinRat.fprintfh highlight ppf c
 | _ -> Format.fprintf ppf "(%a)+sqrt(2)(%a)"
   (BinRat.fprintfh highlight) c (BinRat.fprintfh highlight) d
;;
...
let use_fresh_bits (c,d) = 
 liftM2 Pair.make (BinRat.use_fresh_bits c) (BinRat.use_fresh_bits d)

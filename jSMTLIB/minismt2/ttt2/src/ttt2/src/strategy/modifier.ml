 ...
 * On the one hand it must be guaranteed that each processor has the same
 ...
 * termination problem. This means you have to specify if it is terminating,
 * nonterminating or open. If your processor failed you have to indicate this
...
let apply solve get_problem make =
 ...
 M.return2 s (Option.fold get_problem (S.get_problem s) r) proof status
;;

let applym solve get_problem make =
 M.get >>= fun s -> M.lift (solve (S.get_problem s)) >>= fun r ->
 let (proof,status) = option (unfinished <.> make) failed r in
 M.return2 s (Option.fold get_problem (S.get_problem s) r) proof status
;;

...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
    ...
    <xsl:template match="distinctNormalForms">
        they are distinct normal forms.
    </xsl:template>
    
    <xsl:template match="capNotUnif">
        when applying the cap-function on both sides (where variables may be treated like constants)
                ...
                <table align="center">            
                    ...
                    <xsl:for-each select="equationStep">
                        <tr>
                           <td>
                               <xsl:choose>
                                   <xsl:when test="leftRight"> &#8594; </xsl:when> 
                                   <xsl:when test="rightLeft"> &#8592; </xsl:when>                                    
                               </xsl:choose>  
                           </td>
    ...
    <xsl:template match="joinableCriticalPairsBFS">
        ...
        <xsl:value-of select="text()"/> steps.
    </xsl:template>
    ...
    <xsl:template match="patternRule">
        <b><xsl:apply-templates select="patternTerm[1]"/>&#160;&#8594;<sup>+</sup>&#160;<xsl:apply-templates select="patternTerm[2]"/></b><br/>
        <xsl:choose>
    ...
    <xsl:template match="loop" mode="nonterm">
        ...
        <p>
            ...
            <xsl:if test="$subst">&#963;</xsl:if>
            <xsl:if test="$context">]</xsl:if>
            <xsl:if test="$subst or $context">
                and
            </xsl:if>
            <xsl:if test="$subst">
                &#963; = <xsl:apply-templates select="substitution"/>
            </xsl:if>
            ...
            <tr>
                <xsl:choose>
                    <xsl:when test="count(*) = 4">
                        <td><xsl:apply-templates select="rule[1]/lhs"/></td>
                        <td> &#8594; </td>
                        <td><xsl:apply-templates select="rewriteStep/*[last()]"/></td>                       
                    </xsl:when>
                    <xsl:when test="count(*) = 5">
                        <td><xsl:apply-templates select="rule[2]/lhs"/></td>
                        <td> &#8594; </td>
                        <td><xsl:apply-templates select="rule[2]/rhs"/></td>                        
                    </xsl:when>
                </xsl:choose>
    ...
    <xsl:template match="argumentFilter">
      <xsl:choose>
        <xsl:when test="count(argumentFilterEntry) &gt; 1">
          <p>
          <table>
              <xsl:for-each select="argumentFilterEntry">
          <tr>
            <td align="right">&#960;(<xsl:apply-templates select="*[1]"/>)</td>
            <td align="center">=</td>
            <td align="left">
                <xsl:choose>
        ...
        <xsl:otherwise>
            &#960;(<xsl:apply-templates select="argumentFilterEntry/*[1]"/>)
            =
            <xsl:value-of select="argumentFilterEntry/collapsing"/>
        </xsl:otherwise>
      </xsl:choose>
    </xsl:template>
    ...
    <xsl:template match="relativeTerminationAssumption">
      <xsl:param name="indent"/>
      <h3><xsl:value-of select="$indent"/> Termination Assumption</h3>
    ...
    <xsl:template match="nonterminationAssumption">
      <xsl:param name="indent"/>
    ...
    <xsl:template match="constraint" mode="cc">
        <xsl:apply-templates select="*[1]"/>
        <xsl:choose>
            <xsl:when test="nonStrict"> &#8805; </xsl:when>
            <xsl:when test="rewrite"> &#8594;<sup>*</sup> </xsl:when>
    ...
    <xsl:template match="equalityRemoval" mode="relative">
        ...
        <p>All equality rules (of the form t &#8594; t) are removed from the non-strict rules. </p>
        <p>
            <xsl:apply-templates select="relativeTerminationProof">
                <xsl:with-param name="indent" select="concat($indent, '.1')"/>
            </xsl:apply-templates>

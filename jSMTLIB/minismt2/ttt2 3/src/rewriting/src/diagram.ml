...
module Make (L : LABEL) = struct
 ...
 let label label_step n d = mapl label_step n d;;
 ...
 let lex_max = List.foldl1 (fun a b -> if lex_gt a b then a else b);;

 let is_decreasing_l_strict alpha beta ls = 
 ...
 let rec is_decreasing_l alpha beta = function
  ...
  | l::ls when lex_eq beta l -> is_decreasing_l_strict alpha beta ls
  | ls -> is_decreasing_l_strict alpha beta ls
 ;;

 let is_decreasing_s dec alpha beta ss = 
  dec alpha beta (List.map Rewrite.step_get_lab ss)
 ;;

 let is_decreasing_j dec alpha beta (ls,rs) =
  is_decreasing_s dec alpha beta ls &&
  is_decreasing_s dec beta alpha rs
 ;;

 let is_decreasing_aux dec d = 
  let (l,r) = peak d in
  let (nl,nr) = (Rewrite.step_get_lab l, Rewrite.step_get_lab r) in
  List.exists (is_decreasing_j dec nl nr) (joins d)
  ;;
 ...
 let rec is_par_decreasing_p plab alpha beta vs ps = function
 | [] -> true
 | x::xs when List.for_all (Position.are_parallel (Rewrite.step_get_pos x)) ps
           && List.is_subset (step_get_vars x) vs
  -> if lex_eq beta (Rewrite.step_get_lab (plab x)) ||
        lex_gt beta (Rewrite.step_get_lab x)
     then is_par_decreasing_p plab alpha beta vs (Rewrite.step_get_pos x::ps) xs
     else is_decreasing_l_strict alpha beta (List.map Rewrite.step_get_lab (x::xs))
 | xs -> is_decreasing_l_strict alpha beta (List.map Rewrite.step_get_lab xs)
 ;;

 let rec is_par_decreasing_l f n alpha beta vs = function
 | [] -> true
 | x::xs when lex_gt alpha (Rewrite.step_get_lab x) ->
  is_par_decreasing_l f (Pair.fst (f n x)) alpha beta vs xs
 | (x::_) as xs ->
  let s = Rewrite.step_get_term x in
  let plab x = Pair.snd (f n (Rewrite.step_set_term s x)) in
  is_par_decreasing_p plab alpha beta vs [] xs
 ;;

 let is_par_decreasing_j f nl alpha nr beta vs (ls,rs) =
  is_par_decreasing_l f nl alpha beta [] ls &&
  is_par_decreasing_l f nr beta alpha vs rs
 ;;

 let is_par_decreasing f n d =
  let (ls,r) = par_peak d in
  let alpha = lex_max (List.map Rewrite.step_get_lab ls) in
  let beta = Rewrite.step_get_lab r in
  ...
  List.exists (is_par_decreasing_j f nl alpha nr beta vs) (joins d)
 ;;

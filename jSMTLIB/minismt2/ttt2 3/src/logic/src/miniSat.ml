...
let send_clause s = function
 ...
 | xs  ->
   (* let _ = List.fprintf Int.fprintf " " Format.std_formatter xs in *)
...
let solve_generic solvefun f =
  let _ = print_stat (fun _ -> "start solving") in
 tp (return f) >>= fun f -> 
 get_side_constraint () >>= fun sc ->
  let _ = print_stat (fun _ -> "propositional formula generated") in
 let phi = f <&> sc in
  let _ = print_stat (fun _ -> 
   (Format.sprintf "%d vars" (List.length (Formula.p_vars_tmp phi)))) in
  let _ = print_formula phi in
  let _ = print_stat (fun _ -> "start cnf transformation") in

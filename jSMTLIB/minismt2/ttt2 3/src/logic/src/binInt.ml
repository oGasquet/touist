...
let to_binNat (sign,bits) = bits@[sign];;
let of_binNat bn = (List.last bn, List.init bn);;

(*** conversions **************************************************************)
let fprintfh highlight ppf (sign, nat) =
 match sign with
 | Formula.Bot -> Format.fprintf ppf "%a" (BinNat.fprintfh highlight) nat
 | Formula.Top -> Format.fprintf ppf "-%a" (BinNat.fprintfh highlight) nat
 | _ -> Format.fprintf ppf "sign:%a %a"
   (BinNat.fprintfh_p highlight) sign (BinNat.fprintfh highlight) nat
;;

let fprintf = fprintfh (fun _ -> false);;
...
let restrict obits a = 
 ...
 M.add_overflow_constraint (big_and (List.map ((<<->>) (sign a)) o)) >>
 ...
 *)
 (**)
 BinNat.add (to_binNat a) (to_binNat b) >>= fun s -> 
 let r = of_binNat (List.init s) in
 (**)
 restrict obits r
;;
...
let mul ?(obits=max_int) a b = if sign a = bot && sign b = bot
 ...
 (**)
 (*HZ improved by working on (significantly) fewer bits*)
 (*
 let m = List.length (BinNat.bits (bits a)) in
 let n = List.length (BinNat.bits (bits b)) in
 let a = sign_extend2 (n) a in
 let b = sign_extend2 (m) b in
 BinNat.mul_aux max_int (bits a) (bits b) >>= fun prod ->
 let prod = List.take (m+n) (BinNat.bits prod) in
 *)
...
let rec scale n a =
 ...
 if (sign a = bot) && Integer.gt n Integer.zero 
  then liftM2 make (return (sign a)) (BinNat.scale n (bits a)) else 
 mul (of_int n) a;;
...
let fresh spec = BinNat.fresh spec >>= fun bits ->
 ...
 (* to have non-empty unsat core for BinNat.t when using constants *)
 (* fresh_bool >>= fun sign -> *)
 (* (if spec.neg then return () else M.add_side_constraint (sign <<->> bot)) >> *)

...
(*** FUNCTIONS ****************************************************************)
type solver = MiniSat | MiniSatP | MiniSmt of string list | Yices | PicoSat;;
...
let solve2 ?(solver=MiniSat) ?(goal=None) f =
 ...
 *)
 ...
 (match solver with
  | PicoSat -> PicoSat.solve f
  | MiniSat -> MiniSat.solve f >>= either_of_option
...
let solve ?(solver=MiniSat) ?(goal=None) f =
 solve2 ~solver:solver ~goal:goal f >>= function

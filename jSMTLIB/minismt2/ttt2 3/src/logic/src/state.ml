...
open Util;;
 ...
 type t = {
  ...
  epy_tbl : (Formula.p, bool) Hashtbl.t;     (* chache evaluation yices *)
  sc  : (Formula.p, unit) Hashtbl.t;     (* side conditions *)
  overflow_constraints : (Formula.p, unit) Hashtbl.t;
 ...
 let init dbits obits = {
   ...
   overflow_constraints = Hashtbl.create 512;
   dbits = dbits;
   obits = obits;
   count = ref 0;

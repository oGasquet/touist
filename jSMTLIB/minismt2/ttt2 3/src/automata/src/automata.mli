...
module type MONAD = sig
 ...
 val lift : 'a m -> 'a t
 (** [lift m] is equivalent to
 {!val: Util.Monad.Transformer.COMBINED_MONAD.lift}. *)
...
module type AUTOMATON = sig
 ...
 val qdet : t -> (t * (State.t * State.t) list)
 (** [qdet a] returns a deterministic, q-compatible automaton equivalent to
 [a] provided that [a] is quasi-deterministic.  This may lead to an exponential
 blowup. The function also returns the used precedence. *)

 (** {3 Reachability Analysis} *)

 val compatible : ?c:bool -> ?qc:bool -> ?h:t option -> ?t:(term -> term -> term m)
  -> trs -> t -> (term * State.t) list m
 (** [compatible ~c:c ~qc:qc ~h:h ~t:t r a] computes all compatibility
 violations with respect to the TRS [r] and the tree automaton [a]. If [c] is
 set to true, the internal categorization is used. In that case it might
 happen that fewer violations are obtained because transitions are applied
 which would be in general not applicable. If [h] is specified, it is
 guaranteed that in each derivation at least one transition of [h], that is
 also contained in [a], is used. Via the function [t] it is possible to
 modify the right-hand sides involved in compatibility violations. As input
 the function [t] gets the left- and right-hand side of the considered rewrite
 rule (not that in case that [c] is true it might happen that the left-hand
 side has been changed). If [qc] is set, then the categorization is used
 on the right-hand side of potential violations. Note that as a side effect
 the old signature is changed too. Not tail-recursive. *)
 ...
 val is_det : t -> bool
 (** [is_det t] checks whether [t] is deterministic *)
 val is_quasi_det : t -> bool
 (** If [is_quasi_det t] returns true then [t] is quasi-deterministic.
  [is_quasi_det] returns true for the results of [quasi_det].
  The function may return [false] for quasi-deterministic automata. *)

...
module Make (R : REWRITING)
 ...
 let torpa ?(c = false) f t p a =
  M.liftM Term.choose (A.rewrite ~c:c t a) >>= fun u ->
 ...
 let rewrite c t a = match t with
  ...
  | Fun (f,ts) ->
   M.map (M.liftM Term.choose <.> flip (A.rewrite ~c:c) a) ts >>= fun ts ->
   M.return (Fun (f,ts))
 ;;

 let suffix ?(p = false) ?(c = false) f t q a =
  (if p then rewrite c t a
  else M.liftM Term.choose (A.rewrite ~c:c t a)) >>= fun u -> f u q a
 ;;

 let tttbox ?(p = false) ?(n = 1) ?(c = false) f t q a =
  (if p then rewrite c t a
  else M.liftM Term.choose (A.rewrite ~c:c t a)) >>= fun u ->
  A.context ~c:c ~n:n u q a >>= M.foldl (fun a (v,p) -> f v p a) a
 ;;

 let ttt2 ?(p = false) ?(n = 1) ?(c = false) f t q a =
  let choose = M.liftM Term.choose in

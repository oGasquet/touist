...
type t =
 ...
 | P_dec of Confluence.Decreasing.t
 | P_groundcr of Confluence.GroundCR.t
 (* FIXME delete processor P_cf *)
 | P_dp of Dp.t
 | P_edg of Dg.Edg.t
 | P_eps0 of EpsilonZero.t
 | P_fbi of Fbi.t
 | P_lpo of Lpo.t
 | P_kbo of Kbo.t
 | P_tkbo of Tkbo.t
 | P_matrix of Matrix.t
 | P_ordinal of Ordinal.t
 | P_sorted of Sorted.t
 | P_poly of Poly.t
 | P_sc of SC.t
 | P_sccs of Sccs.t
 | P_sct of SCT.t
 ...
 | P_emb of Emb.t
 | P_co of Co.t
 | P_cl of Confluence.Closed.t
 | P_aotoToyama of Confluence.AotoToyama.t
 | P_kleinHirokawa of Confluence.KleinHirokawa.t
 | P_cr of Cr.t
...
let input = function
 ...
 | P_tkbo p -> [Tkbo.get_ip p]
 | P_matrix p -> [Matrix.get_ip p]
 | P_ordinal p -> [Ordinal.get_ip p]
 | P_sorted p -> [Sorted.get_ip p]
 ...
 (* transformation processors *)
 | P_cp p -> [Cp.get_ip p]
 | P_dpify p -> [Dpify.get_ip p]
 | P_emb p -> [Emb.get_ip p]
 | P_co p -> [Co.get_ip p]
 | P_cl p -> [Confluence.Closed.get_ip p]
 | P_aotoToyama p -> [Confluence.AotoToyama.get_ip p]
 | P_kleinHirokawa p -> [Confluence.KleinHirokawa.get_ip p]
 | P_cr p -> [Cr.get_ip p]
...
let output = function
 ...
 | P_cfs p -> [Cfs.get_ip p]
 (* FIXME delete processors *)
 | P_rl p -> [Confluence.RuleLabeling.get_op p]
 | P_shift p -> [Confluence.Shift.get_op p]
 | P_dec p -> [Confluence.Decreasing.get_op p]
 | P_groundcr p -> Result.get_ops (Confluence.GroundCR.get_result p)
 (* FIXME delete processors *)
 | P_dp p -> [Dp.get_op p]
 | P_edg p -> [Dg.Edg.get_op p]
 | P_eps0 p -> [EpsilonZero.get_op p]
 | P_fbi p -> [Fbi.get_op p]
 | P_lpo p -> [Lpo.get_op p]
 | P_kbo p -> [Kbo.get_op p]
 | P_matrix p -> [Matrix.get_op p]
 | P_tkbo p -> [Tkbo.get_op p]
 | P_ordinal p -> [Ordinal.get_op p]
 | P_sorted p -> Sorted.get_ops p
 ...
 | P_emb p -> [Emb.get_op p]
 | P_co p -> [Co.get_op p]
 | P_cl p -> [Confluence.Closed.get_op p]
 | P_aotoToyama p -> [Confluence.AotoToyama.get_op p]
 | P_kleinHirokawa p -> [Confluence.KleinHirokawa.get_op p]
 | P_cr p -> [Cr.get_op p]
...
let equal p q = match (p,q) with
 ...
 | P_dec p, P_dec q -> Confluence.Decreasing.equal p q
 | P_groundcr p, P_groundcr q -> Confluence.GroundCR.equal p q
 (* FIXME delete processor P_cf *)
 | P_dp p, P_dp q -> Dp.equal p q
 | P_edg p, P_edg q -> Dg.Edg.equal p q
 | P_eps0 p, P_eps0 q -> EpsilonZero.equal p q
 ...
 | P_ordinal p, P_ordinal q -> Ordinal.equal p q
 | P_sorted p, P_sorted q -> Sorted.equal p q
 ...
 | P_dpify p, P_dpify q -> Dpify.equal p q
 | P_emb p, P_emb q -> Emb.equal p q
 | P_co p, P_co q -> Co.equal p q
 | P_cl p, P_cl q -> Confluence.Closed.equal p q
 | P_aotoToyama p, P_aotoToyama q -> Confluence.AotoToyama.equal p q
 | P_kleinHirokawa p, P_kleinHirokawa q -> Confluence.KleinHirokawa.equal p q
 | P_cr p, P_cr q -> Cr.equal p q
...
let complexity c = function
 ...
 | P_ordinal p -> Some (Ordinal.complexity c p)
 | P_sorted p -> Some (Sorted.complexity c p)
 ...
 (* | P_emb p -> Some (Emb.complexity c p) *)
 (* | P_cl p -> Some (Confluence.Closed.complexity c p) *)
 (* | P_cr p -> Some (Cr.complexity c p) *)
 (* | P_co p -> Some (Co.complexity c p) *)
...
let fprintf fs fmt = function
 ...
 | P_dec p -> Confluence.Decreasing.fprintf fs fmt p
 | P_groundcr p -> Confluence.GroundCR.fprintf fs fmt p
 (* FIXME delete processor P_cf *)
 | P_dp p -> Dp.fprintf fs fmt p
 | P_edg p -> Dg.Edg.fprintf fs fmt p
 | P_eps0 p -> EpsilonZero.fprintf fs fmt p
 ...
 | P_ordinal p -> Ordinal.fprintf fs fmt p
 | P_sorted p -> Sorted.fprintf fs fmt p
 ...
 | P_emb p -> Emb.fprintf fs fmt p
 | P_co p -> Co.fprintf fs fmt p
 | P_cl p -> Confluence.Closed.fprintf fs fmt p
 | P_aotoToyama p -> Confluence.AotoToyama.fprintf fs fmt p
 | P_kleinHirokawa p -> Confluence.KleinHirokawa.fprintf fs fmt p
 | P_cr p -> Cr.fprintf fs fmt p
...
(* XML printer *)
let printx s f fs p =
  if Status.is_nonterminating s then XO.seq fs else f fs p
;;

let fprintfx s fs = function
 (* modifiers *)
 | P_restore p -> printx s Restore.fprintfx fs p
 (* nontermination processors *)
 | P_con p -> Contained.fprintfx fs p
 | P_loop p -> LoopSat.fprintfx fs p
 | P_unfold p -> Unfolding.fprintfx fs p
 | P_var p -> Variables.fprintfx fs p
 (* termination processors *)
 | P_adg p -> printx s Dg.Adg.fprintfx fs p
 | P_arctic p -> printx s Arctic.fprintfx fs p
 | P_bounds p -> printx s Bounds.fprintfx fs p
 | P_cdg p -> printx s Dg.Cdg.fprintfx fs p
 | P_cfs p -> printx s Cfs.fprintfx fs p
 (* FIXME delete processor P_cf *)
 | P_ncf p -> Confluence.Nonconfluence.fprintfx fs p
 | P_rl p -> printx s Confluence.RuleLabeling.fprintfx fs p
 | P_shift p -> printx s Confluence.Shift.fprintfx fs p
 | P_dec p -> printx s Confluence.Decreasing.fprintfx fs p
 | P_groundcr p -> printx s Confluence.GroundCR.fprintfx fs p
 (* FIXME delete processor P_cf *)
 | P_dp p -> printx s Dp.fprintfx fs p
 | P_edg p -> printx s Dg.Edg.fprintfx fs p
 | P_eps0 p -> printx s EpsilonZero.fprintfx fs p
 | P_fbi p -> printx s Fbi.fprintfx fs p
 | P_lpo p -> printx s Lpo.fprintfx fs p
 | P_kbo p -> printx s Kbo.fprintfx fs p
 | P_tkbo p -> printx s Tkbo.fprintfx fs p 
 | P_matrix p -> printx s Matrix.fprintfx fs p
 | P_ordinal p -> printx s Ordinal.fprintfx fs p
 | P_sorted p -> printx s Sorted.fprintfx fs p
 | P_poly p -> printx s Poly.fprintfx fs p
 | P_sc p -> printx s SC.fprintfx fs p
 | P_sccs p -> printx s Sccs.fprintfx fs p
 | P_sct p -> printx s SCT.fprintfx fs p
 (* | P_sl p -> printx s SL.fprintfx fs p *)
 | P_tdg p -> printx s Dg.Tdg.fprintfx fs p
 | P_trivial p -> printx s Trivial.fprintfx fs p
 (* transformation processors *)
 | P_cp p -> printx s Cp.fprintfx fs p
 | P_dpify p -> printx s Dpify.fprintfx fs p
 | P_emb p -> printx s Emb.fprintfx fs p
 | P_linear p -> printx s Linear.fprintfx fs p
 | P_qlab p -> printx s QLab.fprintfx fs p
 | P_ref p -> printx s Reflect.fprintfx fs p
 | P_rev p -> Reverse.fprintfx fs p
 | P_rlab p -> printx s RLab.fprintfx fs p
 | P_rt p -> printx s Rt.fprintfx fs p
 | P_split p -> printx s Split.fprintfx fs p
 | P_st p -> printx s St.fprintfx fs p
 (* | P_ti p -> printx s Ti.fprintfx fs p *)
 | P_uncurry p -> printx s Uncurry.fprintfx fs p
 | P_uncurryx p -> printx s Uncurryx.fprintfx fs p
 | P_ur p -> printx s Ur.fprintfx fs p
 | P_cr p -> printx s Cr.fprintfx fs p
 | P_cl p -> printx s Confluence.Closed.fprintfx fs p
 | _ -> failwith "XML output is not supported for this method"
;;

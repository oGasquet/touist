...
(*** GLOBALS ******************************************************************)
let code = "sorted";;
let name = "Sordedness Decomposition";;
...
let order_sorted p = 
 ...
 let step rule (i, cs) =
   ...
   let rec inner strict t cs =
     ...
     match t with
       | Term.Var v -> cs
       | Term.Fun (f, ts) -> snd (List.fold_left (fun (i, cs) t ->
           (i+1, inner0 (Arg (f, i)) t cs)) (1, cs) ts)
   in
 ...
 let (_, cs) = Trs.fold step (1, []) trs in
 ...
 let cs = if not strict then cs else
   ...
   snd (Trs.fold step (1, cs) trs) in
 (* add syntactical constraints *)
 let funs = Trs.funs (P.get_trs p) in
 M.map M.find_ari funs >>= fun ari ->
 let arimap = List.map2 (fun a b -> (a, b)) funs ari in
 let addari cs (f, n) = List.map (fun i -> (Fun f, Arg (f, i))) (List.range 1 (n+1)) @ cs in
 let cs = List.fold_left addari cs arimap in
 (* solve *)
 let g = G.make [] cs in
...
*)
 ...
 (* extract signature *)
 let h = H.of_list (snd (List.fold_left (fun (i, ps) scc -> (i + 1, List.map (fun a -> (a, i)) scc @ ps) ) (1, []) sccs)) in
 let signature = flip List.map arimap (fun (f, n) ->
   (f,
    List.map (fun i -> H.find_ (Arg (f, i)) h) (List.range 1 (n+1)),
    H.find_ (Fun f) h)) in
...
let fprintfm_proof fmt p =
  ...
  Format.fprintf fmt "so@[rt attachment (%s)" (if p.strict then "strict" else "non-strict");

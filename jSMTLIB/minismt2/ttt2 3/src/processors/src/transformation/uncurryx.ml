...
let fprintf_info fmt p =
  ...
  F.fprintf fmt "@]";
  F.fprintf fmt "@\n@[<1>uncurry-rules:@\n";
  Trs.fprintfm fmt p.uncurry_rules >>= fun _ ->
  F.fprintf fmt "@]";
  F.fprintf fmt "@\n@[<1>eta-rules:@\n";
...
let eta_rules_trs o tab trs =
  M.liftM Trs.of_list (M.flat_map
...
let fun_k f i =
  M.find_ari f >>= fun n ->
  M.is_dp f    >>= (function
    | false -> M.set_curry ~arity:(n+i) f i
    | true  -> M.drop_dp f                  >>= fun f ->
               M.set_curry ~arity:(n+i) f i >>= fun f ->
               M.set_dp f)
;;
(*
let fun_k f i =
...
let fun_k_top o f i = if i = 0 then (
  ...
  M.to_string_fun f         >>= fun id  ->
  M.to_string_fun o         >>= fun ap  ->
...
let uncurrying_rules o tab fs =
  M.liftM Trs.of_list (M.flat_map (uncurry_rules_fun o tab) fs);;
...
let uncurrying_rules_top o tab = M.liftM Trs.of_list (uncurry_rules_fun_top o tab);;
...
let find_appsym_with_arity_filter p get fs trs =
  weighted_funas fs trs >>= (M.return <.> (get <.> List.filter p))
;;


let get_first = function (f,_)::_ -> Some f | _ -> None;;
let get_unique = function [(f,_)] -> Some f | _ -> None;;
let find_most_frequent_binary_appsym = fun fs dps ->
  find_appsym_with_arity_filter (fun (o,a) ->
    a = 2 && is_left_head_var_free o dps) get_first fs dps
;;
let find_most_frequent_appsym = fun fs dps ->
  find_appsym_with_arity_filter (fun (o,a) ->
    a > 0 && is_left_head_var_free_top o dps) get_first fs dps
;;
...
let symtab_for_minimal_lhs_aarity_gen unapp aarity maximal o trs =
  maximal o trs >>= fun tab ->
  let tab = Trs.fold (fun rule tab ->
    let l                    = Rule.lhs rule in
    let (Term.Fun (f, _), _) = unapp o l in
    let Some aa              = aarity o tab l in
...
let symtab_for_minimal_lhs_aarity =
  symtab_for_minimal_lhs_aarity_gen
    unapply aarity_term symtab_for_maximal_occurring_aarity;;
let symtab_for_minimal_lhs_top_aarity =
  symtab_for_minimal_lhs_aarity_gen
    unapply_top aarity_term_top symtab_for_maximal_occurring_top_aarity;;
...
let uncurry_dp_top find_o mk_symtab p =
  ...
  find_o sharps dps >>= function None -> M.return None | Some o -> (
    if not (is_left_head_var_free_top o dps) then M.return None else (
      ...
      M.exists (M.liftM Term.is_var <.>
        flip Trs.etcap trs <.> fst <.> unapply_top o <.> Rule.rhs)
        (Trs.to_list dps) >>= (function
        | false -> M.return (uncurry_trs_top o tab dps, trs, Trs.empty, Trs.empty)
        | true  -> (* not root-stable *)
          ...
          let us' = apply_symtab_trs o tab us in
          M.return (dps', trs, us', eta)
      ) >>= fun (dps', trs', us, eta) ->
      let p'   = Problem.set_sw dps' trs' p in
      let info = { appsym = o; symtab = tab } in
      M.return (Some (info, p', us, eta))
    )
  )
;;
...
let uncurry_rp find_o mk_symtab p =
  let (s, w) = Problem.get_sw p in
  let b      = Trs.union s w in
  let fs     = Trs.funs b in
  find_o fs b >>= function None -> M.return None | Some o -> (
    if not (is_left_head_var_free o b) ||
      Trs.exists (Term.is_var <.> Rule.lhs) w then M.return None else (
      let fs' = List.remove o fs in
      mk_symtab o b              >>= fun tab   ->
      eta_rules_trs o tab s      >>= fun eta_s ->
      eta_rules_trs o tab w      >>= fun eta_w ->
      uncurrying_rules o tab fs' >>= fun us    ->
      let s'   = uncurry_trs o tab (Trs.union s eta_s) in
      let w'   = Trs.union (uncurry_trs o tab (Trs.union w eta_w)) us in
      let p'   = Problem.set_sw s' w' p in
      let info = { appsym = o; symtab = tab } in
      M.return (Some (info, p', us, Trs.union eta_s eta_w))
    )
  )
;;
...
let solve_aux find_o mk_symtab top p =
  ...
  else if Problem.is_rp p then uncurry_rp find_o mk_symtab p
  else M.return None
;;
...
(* XML Printing *)
let fprintfx_entry f k fs = XO.node "uncurriedSymbolEntry" (
  M.fprintfx_fun f 
  :: XO.int "arity" k
  :: List.map M.fprintfx_fun fs)
;;

let fprintfx_symtab tab = XO.seq (List.map (fun (f, fs) fmt ->
  M.find_ari f >>= fun k ->
  fprintfx_entry f k fs fmt) (FunMap.to_list  tab))
;;

let fprintfx_info p = XO.node "uncurryInformation" [
  M.fprintfx_fun p.info.appsym;
  XO.node "uncurriedSymbols" [fprintfx_symtab p.info.symtab];
  XO.node "uncurryRules"     [Trs.fprintfx p.uncurry_rules];
  XO.node "etaRules"         [Trs.fprintfx p.eta_rules]]
;;

let fprintfx_applicative_top p fmt =
  if p.applicative_top then (
    M.find_ari p.info.appsym >>= fun n ->
    XO.int "applicativeTop" n fmt
  ) else XO.id fmt
;;

let fprintfx fs p =
  let op = get_op p in
  XO.node (if Problem.is_dp op then "uncurryProc" else "uncurry") [
    fprintfx_applicative_top p;
    fprintfx_info p;
    Problem.fprintfx op;
    List.hd fs]
;;

...
let fprintfx_dom p =
  let dom_arctic =
    XO.node "domain" [XO.node "arctic" [XO.node "domain" [XO.leaf "integers"]]]
  in
  if p.dimension = 1
    then dom_arctic
    else
      XO.node "domain" [XO.node "matrices" [
        XO.int "dimension" p.dimension;
...
let fprintfx fs p = if Problem.is_dp p.input then (
  ...
  failwith "ArcticInterpretation.fprintfx: XML output not supported!"
)
;;
...
(* not zero matrix *)
let nzm f i = liftM Logic.neg (zero_matrix (coefficient f i));;
...
let geq_ur op geq rule =
 liftM2 op (u(Rule.lhs rule)) (geq rule)
;;
...
let decode_rule ass rule = 
 ...
 lift (Logic.eval_p enc ass) >>= (return <.> not)
;;
...
let decode_ur_rule ass rule =
 u (Rule.lhs rule) >>= fun u -> lift (Logic.eval_p u ass)
;;

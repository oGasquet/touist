...
let compatible f g = get >>= fun c ->
 ...
 return () else (
 (* Format.printf "variable order not compatible@\n%!"; *)
 get >>= fun c ->
...
let is_zero f =
 ...
 in cache_m c.ht_zero is_zero' f
;;
 ...
 match ff, gg with
  ...
  |Ord f, Ord g ->
   ...
   let lcf x =
    ...
    return (x, max ((s x)<*>(c f.cs x)) ((t x)<*>(c g.cs x))) 
   in
   map lcf xs >>= fun lcs ->
 ...
 (* unsound if f = y and g = x + y?*)
  ...
  | Ord f, Ord g ->
   ...
   let cf x = 
    s x >>= fun sx -> 
    t x >>= fun tx ->
    let a =(sx<*>tx<*>(c f.cs x))<+>(tx<*>(c f.ncs x))<+>(tx<*>(c g.cs x)) in
...
let make_var x = make [] Zero L.zero [x,L.one] L.zero

let fst_m = return <.> fst;;
let snd_m = return <.> snd;;

let rec compose os = function
 | Zero -> return (Zero, Zero)
 | Ord o ->
  let v = false in
  if v then print_inter "COMPOSE: " (Ord o);

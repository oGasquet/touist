...
let fprintfx fs p = XO.node "subtermProc" [
  SP.fprintfx p.sp;
  P.fprintfx p.output;
  List.hd fs];;
...
let collapsing f a i =
  let make j = if i = j then pos f i else (liftM Logic.neg (pos f j)) in
  gen_and make a
;;
...
let decode_rule ass rule =
  get >>= fun c ->
  liftM not (eval_p (gt_rule rule) ass)
;;
...
let solve signature fs p = 
  ...
  if P.is_dp p then (
    ...
    Logic.run (
      ...
      StateM.lift (Logic.solve ~solver:c.solver phi) >>= function
        | None -> return None
        | Some ass ->
          decode_trs ass s >>= fun s' -> 
	  (* Trs.fprintfm Format.std_formatter s' >>= fun _ -> *)

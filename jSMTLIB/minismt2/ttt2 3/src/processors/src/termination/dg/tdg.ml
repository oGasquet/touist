...
let solve fs p =
  ...
  M.return (
    if P.is_dp p then
     ...
     let dps = P.get_dps p and trs = P.get_trs p in
     match P.next_dg p with
      | P.Complete ->
       let g = G.generate generate (Trs.to_list dps) in
       let n = Int.square (Trs.size dps) in
       let q = P.set_dg (P.Partial g) dps trs p in
       if G.size_edges g < n then Some (p,q) else None
      | P.Partial g -> 
       ...
       if G.size_edges h < n then Some (p,q) else None
    else None
  )
;;

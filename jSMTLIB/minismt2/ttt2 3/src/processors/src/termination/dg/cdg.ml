...
module Completion = struct
 ...
 let adapt fs dps trs x =
  let fresh_var _ = M.liftM Term.make_var M.fresh_var in
  let transform = function
   | Term.Var _ as t -> M.return t
   | Term.Fun (f,ts) -> M.liftM (Term.make_fun f) (M.map fresh_var ts)
  in
  let a = get_automaton x in transform (get_rhs x) >>=
  Initial.normal_instances fs dps trs >>=
  (M.liftM (flip set_automaton x) <.> A.inter a)
 ;;
 ...
 let plain i dps trs =
  ...
  let plain x =
   ...
   if get_compatible x then
    ...
    else M.liftM (flip set_automaton x) (A.det (get_automaton x))
   else M.return x
  in
  M.return plain
 ;;
...
let configuration dps trs =
 if !(flags.rfc) && Trs.is_right_linear trs && Trs.is_right_linear dps then
  M.lift (Trs.linear trs) >>= fun trs' ->
  M.lift (Trs.linear dps) >>= fun dps' ->
  M.lift (Trs.sharp (Trs.union dps' trs')) >>= fun (f,sharp) ->
...
let compute p =
 ...
 match g with
  | P.Complete ->
   ...
   generate f m ns >>= fun (h,m) -> M.get >>= fun s ->
   let i = Int.square (Trs.size dps) and j = G.size_edges h in
   ...
   M.return (if j < i then Some (p,proof,q) else None)
  | P.Partial g ->

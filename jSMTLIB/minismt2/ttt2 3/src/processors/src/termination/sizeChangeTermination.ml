...
let fprintfx_edges = XO.seq <.> List.map (fun (i, b, j) ->
  XO.node "edge" [
    XO.int "position" (i+1);
    XO.bool "strict" b;
    XO.int "position" (j+1)]
...
let fprintfx_initial (rule, _, es) = XO.node "sizeChangeGraph" [
  Rule.fprintfx rule;
  fprintfx_edges es];;
...
let solve fs p =
  ...
  M.return (
    if P.is_dp p then (
      let mem = match P.next_dg p with
...
let fprintfx _ p = XO.node "sizeChangeProc" (
  XO.leaf "subtermCriterion"
  :: List.map G.fprintfx_initial (get_graphs p));;

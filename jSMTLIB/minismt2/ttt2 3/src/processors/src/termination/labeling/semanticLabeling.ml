...
let fprintfx _ _ = failwith "SemanticLabeling.fprintfx: XML output not supported";;
...
let decode_interpretation ass p =
 ...
 Made.filter (fun x -> liftM not (is_dp x)) (Trs.funs trs) >>= fun fs ->
...
let rec rsl_term = function
 ...
 | Term.Fun (f,ts) -> liftM2 Term.make_fun (rsl f) (Made.map rsl_term ts)
;;

let rsl_rule rule = 
 liftM2 Rule.of_terms (rsl_term (Rule.lhs rule)) (rsl_term (Rule.rhs rule))
;;

let rsl_trs trs = liftM Trs.of_list (Made.map rsl_rule (Trs.to_list trs));;

let rsl_p p = 
 let (s,w) = Problem.get_sw p in
 liftM3 Problem.set_sw (rsl_trs s) (rsl_trs w) (return p)
;;
...
and etc c = function
 ...
 | Term.Fun (f, ts) ->
  (List.fold_lefti (fun i p t -> 
   liftM2 P.add p (liftM2 P.scale (lab_coefficient f i) (encode_term t)))
  (liftM P.make (sequence [liftM2 Mono.make (lab_constant f) (return [])]))
  ts)
;;
...
let rec label_term t = match t with
 ...
 | Term.Fun (f,ts) -> encode_lab_term t >>= fun l ->
  liftM2 Term.make_fun (set_label f l) (Made.map label_term ts)
;;
...
let label_trs trs = liftM Trs.of_list (Made.map label_rule (Trs.to_list trs));;
...
let af_n f = liftM Logic.neg (af_l f);;
...
let af_collapsing f a i =
 let make j = if i = j then af_p f i else (liftM Logic.neg (af_p f j)) in
 gen_and make a
;;
...
let same_labels one two f g ss ts = 
 liftM2 equal_poly (get_label f) (get_label g) >>= fun eq ->
 lab f >>= fun l -> 
 ((return (eq <|> (~! l)) $->$ two)) $&$
 ((return ((~!eq) <&> l)) $->$ one)
...
let ge_ur op ge rule = liftM2 op (u(Rule.lhs rule)) (lpo_ge rule);;
...
let decode_rule ass rule = get >>= fun c ->
  liftM not (eval_p (lpo_gt rule) ass)
;;

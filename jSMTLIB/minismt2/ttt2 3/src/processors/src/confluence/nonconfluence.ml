...
let grounding_cps tbl (s::_,o,t::_) = 
...
let ncr_tree trs cps = 
 M.map (grounding_cps (Hashtbl.create 512)) cps >>= fun cps ->
 let us = List.foldl (fun us (s,o,t) -> s::t::us) [] cps in
 ...
 filter f m cps >>= fun (cp,m) -> M.get >>= fun s -> match cp with
  ...
  | Some (l,s,r) ->
   ...
   let proof = make_proof ([l],s,[r]) in (* FIXME: wrong rewrite sequences *)
   M.return (Some proof)
;;
...
let find f trs cps =
 mfind (f trs) cps >>= function
  | None -> M.return None
  | Some cp -> M.return (Some (make_proof cp))
;;
...
let fprintfx fs p =
  ...
  XO.node "crDisproof" [
    XO.node "nonJoinableFork" [
      ...
      if (get_proof p).ptcap then XO.node "capNotUnif" [] else XO.node "magic" []
    ]
  ]
;;

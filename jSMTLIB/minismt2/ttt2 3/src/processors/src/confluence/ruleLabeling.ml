...
let var_step s = var_rule (Rewrite.step_get_rule s) >>= fun l -> 
 let ls = List.map Logic.of_int (Rewrite.step_get_lab s) in
 M.return (if !(flags.left) then l::ls else ls@[l])
;;

let lex_eq ls rs = ls >>= fun ls -> rs >>= fun rs ->
 M.return (List.foldr2 (fun l r acc -> (l <=> r) <&> acc ) Logic.top ls rs)
;;

let lex_gt ls rs = ls >>= fun ls -> rs >>= fun rs -> 
 M.return (List.foldr2 (fun l r acc -> (l <>> r) <|> ((l <=> r) <&> acc)) Logic.bot ls rs)
;;

let eq alpha s = lex_eq (var_step alpha) (var_step s);;
let gt alpha s = lex_gt (var_step alpha) (var_step s);;
...
let label_step left rl _ s = 
 let l = List.assoc (Rewrite.step_get_rule s) rl in
 (l,Rewrite.step_add_label ~left:left s l)
;;
 ...
 Monad.return (Logic.run ~obits:(-1) (
  ...
  M.lift (Logic.solve ~solver:Logic.MiniSat phi) >>= function
   ...
   | Some ass ->
    ...
    decode rl p >>= fun p' -> 
    M.return (Some (make p rl p' !(flags.left)))
  )))

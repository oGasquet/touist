...
let sequence_labeling =
 List.foldr (fun (f,n) (fs,ns) ->
  ((fun (n::ns) s -> let (ns',s') = fs ns s in
                     let (n',s'') = f n s' in
                     ((n'::ns'),s''))),
   (n::ns))
  ((fun _ s -> ([],Rewrite.step_clear_lab s)),[])
;;

let solve p =
 if P.is_ddp p then
  let cds = P.get_cds p and (f,ns) = sequence_labeling (P.get_labeling p) in
  let (s,_) = P.get_sw p in
  let decreasing = if !(flags.par) then Diagram.is_par_decreasing f ns
                   else if !(flags.strict) then Diagram.is_decreasing_strict 
                   else Diagram.is_decreasing in
